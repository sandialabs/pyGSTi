...
class DataComparator():
    ...
    def compute_stat_sig_TVDs(self, verbosity=2):
        """
        Todo
        """
        if verbosity >= 2:
        ...
        for key in self.llrs.keys():
            if self.results.hypothesis_rejected[key]:
                ...
                self.sstvds[key] = [len(key),tvd_val]
            else:
                self.sstvds[key] = [len(key),0]
        if verbosity >= 2:
            print("complete.")
...
# #        single_string_thresh = find_thresh(confidence_level,self.num_strs,self.dof)
# #        number_of_significant_sequences = _np.sum(_np.where(self.llrVals>single_string_thresh,1,0))
# #        composite_thresh = find_thresh(confidence_level,1,self.num_strs*self.dof)
# #        composite_score = _np.sum(self.llrVals)
# #        

#     def report(self,confidence_level=0.95,verbosity=1,composite_test_weighting = 0.5):
#         assert composite_test_weighting <= 1.
#         assert composite_test_weighting >= 0.
#         single_weight = 1. - composite_test_weighting
#         if single_weight == 0:
...
#         else:
#             composite_thresh = find_thresh(confidence_level,1. / composite_test_weighting,self.num_strs*self.dof)
#         number_of_significant_sequences = _np.sum(_np.where(self.llrVals>single_string_thresh,1,0))
#         composite_score = _np.sum(self.llrVals)
...
#         self.number_of_significant_sequences = number_of_significant_sequences
