...
class GateSet(object):
    ...
    def num_nongauge_params(self):
        ...
        """
        #DEBUG - for debugging weird CPTP gauge param counting
        # that ended up being a bug in the Lindblad gate Jacobian.
        #dPG = self._buildup_dPG()
        ##print("dPG shape = ",dPG.shape)
        #
        #dP = dPG[:,0:self.num_params()]
        #U,s_dP,Vt = _np.linalg.svd(dP)
        #dG = dPG[:,self.num_params():]
        #U,s_dG,Vt = _np.linalg.svd(dG)
        #
        ##null_dP = _mt.nullspace_qr(dP) #columns are nullspace basis vectors
        #
        #nullsp = _mt.nullspace_qr(dPG) #columns are nullspace basis vectors
        #U,s,Vt = _np.linalg.svd(dPG)
        ##print("dPG svals = ", list(enumerate(s)))
        ##print("dP svals = ", list(enumerate(s_dP)))
        ##print("dG svals = ", list(enumerate(s_dG)))
        ##print("nullsp shape = ",nullsp.shape)
        #nullG = nullsp[self.num_params():,:]
        ##print("nullG shape = ",nullG.shape) # d2, nNullSp
        #
        #for j in range(nullsp.shape[1]): #loop over cols
        #    paramTransWt = _np.linalg.norm( nullsp[0:self.num_params(),j] )
        #    gaugeTransWt = _np.linalg.norm( nullsp[self.num_params():,j] )
        #    #print("DB: col%d has Wts= %g, %g" % (j,paramTransWt,gaugeTransWt))
        #    #if gaugeTransWt < 1e-6:
        #    #    print("DB: col%d has gaugeTransWt=%g!!" % (j,gaugeTransWt))
        #
        #
        #U,s,Vt = _np.linalg.svd(nullG) # U = (d2,d2) s (nNullSp, nNullSp)
        ##print("singular vals of nullG = ",s)
        #V = Vt[0:len(s),:].T
        #gaugeDirs = _np.dot(nullsp[0:self.num_params(),:],V)
        #other = _np.dot(nullG, V)
        #for j in range(len(s)): #loop over cols
        #    paramTransWt = _np.linalg.norm( gaugeDirs[:,j] )
        #    gaugeTransWt = _np.linalg.norm( other[:,j] )
        #    #print("DB2: col%d has Wts= %g, %g" % (j,paramTransWt,gaugeTransWt))
        #    #if gaugeTransWt < 1e-7:
        #    #    print(" spam = ",gaugeDirs[0:7,j])
        #    #    print(" G1.ham = ",gaugeDirs[7:10,j])
        #    #    print(" G1.oth = \n",gaugeDirs[10:19,j].reshape((3,3)))
        #    #    print(" G2.ham = ",gaugeDirs[19:22,j])
        #    #    print(" G2.oth = \n",gaugeDirs[22:31,j].reshape((3,3)))
        #    #    print(" G3.ham = ",gaugeDirs[31:34,j])
        #    #    print(" G3.oth = \n",gaugeDirs[34:43,j].reshape((3,3)))
        #    
        #
        #ret = _np.linalg.matrix_rank(gaugeDirs)
        #U,s,Vt = _np.linalg.svd(gaugeDirs)
        ##print("singular values = ",s)
        ##print("gaugeDirs shape = ",gaugeDirs.shape)
        ##print("%d gauge params, %d non-gauge" % (ret, self.num_params()-ret))
        #
        ##
        ##gaugeDirs = nullsp[0:self.num_params(),:] #cols are gauge directions (but may not all be independent)
        ##print("gaugedirs shape = ",gaugeDirs.shape)
        #
        ##nongaugeDirs = _mt.nullspace_qr(gaugeDirs.T)
        ##  #columns orthogonal to all gauge directions (non-gauge directions)
        ##ret = _np.linalg.matrix_rank(nongaugeDirs)
        ##print("DB: num non-gauge dirs = ", ret)
        ##return self.num_params() - ret
