...
#REMOVED b/c matplotlib dependency dropped and these functions were unused in codebase
# TO REVIVE: port these to current plotting library (plot.ly?)
...
#    return axs
    ...
    def plot_ranges(self, axs=None):
        ...
        clickNums = set()
        for trueGatesetNum, numClicks, _ in self.errorDict:
            clickNums.add(numClicks)
        clickNums = sorted(list(clickNums))
        numClickNums = len(clickNums)
        if axs is None:
