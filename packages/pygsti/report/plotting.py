...
def count_matrix( gateString, dataset, spamlabel, strs, rhoEPairs=None ):
    """
...
def frequency_matrix( gateString, dataset, spamlabel, strs, rhoEPairs=None):
    """
    ...
    numpy array of shape ( len(EStrs), len(rhoStrs) )
        ...
        where gateString is sandwiched between the each (EStr,rhoStr) pair.
    """
    return count_matrix( gateString, dataset, spamlabel, strs, rhoEPairs) / total_count_matrix( gateString, dataset, strs, rhoEPairs)


def probability_matrix( gateString, gateset, spamlabel, strs, rhoEPairs=None):
    """
...
def chi2_matrix( gateString, dataset, gateset, strs, minProbClipForWeighting=1e-4, rhoEPairs=None):
    """
    ...
    numpy array of shape ( len(EStrs), len(rhoStrs) )
        ...
        (i.e. element_ij = gate_string_chi2( rhoStrs[j] + gateString + EStrs[i])
    """
    rhoStrs, EStrs = strs
    chiSqMx = _np.zeros( (len(EStrs),len(rhoStrs)), 'd')
    if gateString is None: return _np.nan*chiSqMx
    cntMx  = total_count_matrix(  gateString, dataset, strs, rhoEPairs)
    for sl in gateset.get_spam_labels():
        probMx = probability_matrix( gateString, gateset, sl, strs, rhoEPairs)
        freqMx = frequency_matrix(   gateString, dataset, sl, strs, rhoEPairs)
        chiSqMx += _tools.chi2fn( cntMx, probMx, freqMx,
                                     minProbClipForWeighting)
    return chiSqMx


def logl_matrix( gateString, dataset, gateset, strs, minProbClip=1e-6, rhoEPairs=None):
    """
...
def generate_zoomed_boxplot(xvals, yvals, xyGateStringDict, subMxCreationFn, strs, 
    ...
    def makeplot(min_clip, max_clip, x,y):
        ...
        color_boxplot( subMxs[iy][ix], fig=fig, axes=ax, 
                      ...
                      xlabels=val_filter(rhoStrs), ylabels=val_filter(EStrs), vmin=minclip, vmax=maxclip, colorbar=False, 
...
def chi2_boxplot( xvals, yvals, xy_gatestring_dict, dataset, gateset, strs,
                  ...
                  save_to=None, ticSize=20, invert=False, rhoEPairs=None):
    """
    ...
    rhoStrs, EStrs = strs
    def mx_fn(gateStr):
        return chi2_matrix( gateStr, dataset, gateset, strs, minProbClipForWeighting, rhoEPairs)
...
def logl_boxplot( xvals, yvals, xy_gatestring_dict, dataset, gateset, strs,
    ...
    rhoStrs, EStrs = strs
    def mx_fn(gateStr):
        return logl_matrix( gateStr, dataset, gateset, strs, minProbClipForWeighting, rhoEPairs)
    return generate_boxplot( xvals, yvals, xy_gatestring_dict, mx_fn, xlabel,ylabel, m,M,
                            scale,prec,title,sumUp,interactive,boxLabels,histogram,histBins,save_to,ticSize,
                            invert, rhoStrs, EStrs, r"$\rho_i$", r"$E_i$")
...
def direct_mc2gst_gateset( gateStringToEstimate, gateStringLabel, dataset, specs, targetGateset, svdTruncateTo=0,
    ...
    rhoStrs, EStrs = _tools.get_spam_strs(specs)
...
def focused_mc2gst_gateset( gateStringToEstimate, gateStringLabel, dataset, specs, startGateset,
    ...
    """
    rhoStrs, EStrs = _tools.get_spam_strs(specs) # LEXICOGRAPHICAL VS MATRIX ORDER
...
def direct_chi2_boxplot( xvals, yvals, xy_gatestring_dict, dataset, directGatesets, strs, xlabel="", ylabel="",
    ...
    rhoStrs, EStrs = strs
    def mx_fn(gateStr):
        return direct_chi2_matrix( gateStr, dataset, directGatesets.get(gateStr,None), strs, minProbClipForWeighting, rhoEPairs)
    return generate_boxplot( xvals, yvals, xy_gatestring_dict, mx_fn, xlabel, ylabel, m,M,
                            scale,prec,title,sumUp,interactive,boxLabels,histogram,histBins,save_to,ticSize,
                            invert, rhoStrs, EStrs, r"$\rho_i$", r"$E_i$",  )
...
def direct_logl_boxplot( xvals, yvals, xy_gatestring_dict, dataset, directGatesets, strs, xlabel="", ylabel="",
    ...
    rhoStrs, EStrs = strs
    def mx_fn(gateStr):
        return direct_logl_matrix( gateStr, dataset, directGatesets.get(gateStr,None), strs, minProbClipForWeighting, rhoEPairs)
    return generate_boxplot( xvals, yvals, xy_gatestring_dict, mx_fn, xlabel, ylabel, m,M,
                            scale,prec,title,sumUp,interactive,boxLabels,histogram,histBins,save_to,ticSize,
                            invert, rhoStrs, EStrs, r"$\rho_i$", r"$E_i$",  )
...
def direct2x_comp_boxplot( xvals, yvals, xy_gatestring_dict, dataset, directGatesets, strs, xlabel="", ylabel="",
    ...
    **repeated twice** (if this data is available), where the probabilities
    ...
    return generate_boxplot( xvals, yvals, xy_gatestring_dict, mx_fn, xlabel, ylabel, m,M,
                            ...
                            save_to,ticSize,invert, rhoStrs, EStrs, r"$\rho_i$", r"$E_i$" )
...
def whack_a_chi2_mole_boxplot( gatestringToWhack, allGatestringsUsedInChi2Opt, 
                           ...
                           interactive=False, boxLabels=True, histogram=False, histBins=50, minProbClipForWeighting=1e-4,
                           save_to=None, ticSize=20, whackWith=10.0, invert=False, rhoEPairs=None):
    """
