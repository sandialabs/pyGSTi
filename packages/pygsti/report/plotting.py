...
def get_gatestring_map(gateString, dataset, strs, fidPairs=None,
                   gatestring_filter=None):
    """ 
    Pre-compute a list of (i,j,gstr) tuples for use in other matrix-
    generation functions.  

    This consolidates all the logic for selecting a subset (via fidPairs,
    ...
    fidPairs : list, optional
        ...
        pairs to include in a result matrix.

    gatestring_filter : list, optional
        If not None, a list of GateString objects specifying which elements
        of result matrices should be computed.  Any matrix entry corresponding to
    ...
    for i,effectStr in enumerate(effectStrs):
        for j,prepStr in enumerate(prepStrs):
            ...
            if dataset is None or gstr in dataset:
                #Note: gatestring_filter trumps fiducial pair filter
                if gatestring_filter is None:
                    if fidPairs is None or (j,i) in fidPairs:
...
def probability_matrices( gatestring_map, gateset, spamlabels, probs_precomp_dict=None):
    """
    ...
    **repeated twice** (if this data is available), where the probabilities
    ...
    def mx_fn(gateStr,x,y):
        ...
        try:
            ...
            gsmap = get_gatestring_map(gateStr*2, dataset, strs, fidPairs,
                                       gatestring_filter)
