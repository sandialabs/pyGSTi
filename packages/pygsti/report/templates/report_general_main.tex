...
\caption{\putfield{tt_bestGatesetSpamBriefTable}{}\textbf{The GST estimate of the SPAM operations}.  Compare to Table \ref{targetSpamTable}.\label{bestGatesetSpamTable}}
\end{center}
\end{table}
...
\caption{\putfield{tt_bestGatesetEvalTable}{}\textbf{Eigenvalues of estimated gates}.  The spectrum (Eigenvalues column) of each estimated gate, along with the eigenvalues of the gate relative to the target gate (Relative Eigenvalues column), defined as $G_{\mathrm{target}}^{-1}\hat{G}$ (see text for details).\label{bestGatesetEigenvalueTable}}
\end{center}
\end{table}

\begin{table}[h]
\begin{center}
\putfield{bestEstimatePolarEvalPlots}{Polar eigenvalue plots for gates placed here}
\caption{\putfield{tt_bestEstimatePolarEvalPlots}\textbf{Polar plots of gate eigenvalues}.  The eigenvalues from Table \ref{bestGatesetEigenvalueTable} plotted over the complex disc.  Large \textbf{black} and \textcolor{blue}{\textbf{blue}} circles plot the spectrum of the \textbf{ideal target gate} and \textcolor{blue}{\textbf{estimated gate}}, respectively.  Small \textcolor{red}{\textbf{red}} circles plot the \textcolor{red}{\textbf{``relative'' eigenvalues}} (see Table \ref{bestGatesetEigenvalueTable}), and \textcolor{darkgreen}{\textbf{green}} circles plot the relative eigenvalues of the gate \textcolor{darkgreen}{\textbf{repeated 10 times}} (to increase visibility).\label{bestGatesetPolarEigvalTable}}
\end{center}
\end{table}
...
\caption{\putfield{tt_bestGatesetChoiEvalTable}{}\textbf{Choi matrix spectrum of the GST estimated gate set}.  The eigenvalues of the Choi representation of each estimated gate.  Unitary gates have a spectrum $(1,0,0\ldots)$, just like pure quantum states.  Negative eigenvalues are non-physical, and may represent either statistical fluctuations or violations of the CPTP model used by GST.\label{bestGatesetChoiTable}}
\end{center}
\end{table}
