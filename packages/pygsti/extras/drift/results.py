...
class DriftResults(object):
    ...
    def get_timesteps(self, seqInd=None):
        ...
        if seqInd is None:
            ...
            timesteps = _np.array(self.timestamps[seqInd][1:]) - \
                _np.array(self.timestamps[seqInd][:self.number_of_timesteps[seqInd] - 1])
    ...
    def add_spectra(self, frequenciesInHz, spectra, transform, modes=None, overwrite=False):
        ...
        """
        if not overwrite:
            assert(not hasattr(self, "spectra")
                   ), "There are already spectra saved! To overwrite them you must set `overwrite` to True!"
    ...
    def get_drift_frequency_indices(self, entity='avg', sequence='avg', outcome='avg', sort=True, detectorkey=None):
        ...
        if testclasstup not in self._testClasses[detectorkey]:
            ...
            assert(equivtestclasstup in self._testClasses[detectorkey]
                   ), "Drift dectection on this level was not performed! So drift indices cannot be returned."
            testclasstup = equivtestclasstup

        # Todo : here it should for equivalent test pointers.
        assert(testclasstup in self._testClasses[detectorkey]
               ), "Drift dectection on this level was not performed! So drift indices cannot be returned."
    ...
    def get_drift_frequencies(self, entity='avg', sequence='avg', outcome='avg', detectorkey=None):
        ...
        Todo
        """
        freqInd = self.get_drift_frequency_indices(
            entity=entity, sequence=sequence, outcome=outcome, sort=False, detectorkey=detectorkey)
    ...
    def plot_spectrum(self, entity='avg', sequence='avg', outcome='avg',
        ...
        if plotthreshold:
            _plt.axhline(threshold, color='r', label='{} global stat. significance threshold'.format(
                self.significance[detectorkey]))
