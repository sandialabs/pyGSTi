...
def makeAlphaStrListsGxGz(kList):
    ...
    sinStrList = []
    for k in kList:
#        cosStrList += [GST.gatestring.GateString(('Gi','Gx','Gx')+('Gz',)*k + ('Gx','Gx'),
#                                                 'GiGxGxGz^'+str(k)+'GxGx')]#With 'Gi'

        cosStrList += [GST.gatestring.GateString(('Gx','Gx')+('Gz',)*k + ('Gx','Gx'),
                                                 'GxGxGz^'+str(k)+'GxGx')]


        sinStrList += [GST.gatestring.GateString(('Gx','Gx')+('Gz',)*k + ('Gz','Gx','Gx'),
                                                 'GxGxGz^'+str(k)+'GzGxGx')]
...
def makeEpsilonStrListsGxGz(kList):
    ...
    for k in kList:
        epsilonCosStrList += [GST.gatestring.GateString(('Gx',)*k,
                             'Gx^'+str(k))]
    
        epsilonSinStrList += [GST.gatestring.GateString(('Gx','Gx')+('Gx',)*k,
                             'GxGxGx^'+str(k))]
...
def makeThetaStrListsGxGz(kList):
    ...
    for k in kList:
        thetaCosStrList += [GST.gatestring.GateString(('Gz','Gx','Gx','Gx','Gx','Gz','Gz','Gx','Gx','Gx','Gx','Gz')*k,
                             '(GzGxGxGxGxGzGzGxGxGxGxGz)^'+str(k))]
    
        thetaSinStrList += [GST.gatestring.GateString(('Gx','Gx')+('Gz','Gx','Gx','Gx','Gx','Gz','Gz','Gx','Gx','Gx','Gx','Gz')*k,
...
def extractRotationHat(xhat,yhat,k,Nx,Ny,angleName,previousAngle=None):
    """
    ...
    angleName:  The angle (alpha, epsilon, or Phi) to be extracted
    ...
    if angleName == 'alpha':
        arctan2Val = np.arctan2(-(xhat-Nx/2.)/Nx,(yhat-Ny/2.)/Ny)
...
def estAngleList(DS,angleSinStrs,angleCosStrs,angleName):
    """
    ...
    for i in xrange(genNum):
        ...
        angleTemp1 = extractRotationHat(xhatTemp,yhatTemp,2**i,Nx,Ny,angleName,angleTemp1)
...
def estThetaList(DS,angleSinStrs,angleCosStrs,epsilonList,returnPhiFunList = False):
    ...
    PhiList = estAngleList(DS,angleSinStrs,angleCosStrs,'Phi')
    thetaList = []
    PhiFunList = []
...
def analyzeSimulatedRPEExperiment(inputDataset,trueGateset,stringListD):
    ...
    thetaErrorList = []
#    PhiFunErrorList = []
    alphaHatList = estAngleList(inputDataset,alphaSinStrList,alphaCosStrList,'alpha')
    epsilonHatList = estAngleList(inputDataset,epsilonSinStrList,epsilonCosStrList,'epsilon')
