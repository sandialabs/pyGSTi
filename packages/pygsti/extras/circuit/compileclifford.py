...
def compile_clifford(s, p, ds=None, depth_compression=True, algorithm='GGE', 
                                     prefix_paulis=False):
    
    assert(_symp.check_valid_clifford(s,p)), "Input is not a valid Clifford!"
    n = _np.shape(s)[0]//2
    #
    # Todo: update so that device_spec can be none, and then a device spec with CNOT, H, P 
    # and Pauli gates is automatically generated.
    #
    #if ds is not None:
    #assert(device_spec.compilation is not None)
    #assert(device_spec.compilation_pauli is not None)
    #
    # More tests go here
    #
    assert(n == ds.number_of_qubits), "...."
    
    circuit = compile_symplectic(s,ds=ds,algorithm=algorithm,depth_compression=depth_compression)
       
    circuit.change_gate_library(ds.compilations.paulieq)
    
    temp_s, temp_p = _symp.composite_clifford_from_clifford_circuit(circuit,  s_dict=ds.gateset.smatrix,
    ...
    pauli_circuit = _cir.Circuit(gate_list=pauli_layer,n=n)
    ...
    s_out, p_out = _symp.composite_clifford_from_clifford_circuit(circuit, s_dict=ds.gateset.smatrix,
                                                                    p_dict=ds.gateset.svector)
...
def compile_symplectic(s, ds=None, algorithm='GGE', depth_compression=True):    
    """
    
    """
    
    assert(algorithm == 'GGE'), "The only compilation algorithm currently available is Gaussian elimination!"
    
    if algorithm == 'GGE':
        
        circuit = global_gaussian_elimination_on_symplectic(s, ds=ds, 
...
#
# Todo : make this connectivity aware.
#
def global_gaussian_elimination_on_symplectic(M, ds=None, depth_compression=True):
    """
    M: array
    ...
    for j in range (0,d):
        ...
        if j not in upperl_ones:
            ...
            if j in lowerl_ones:
                instruction_list.append(_cir.Gate('H',j))                
            ...
            # Then try using a swap gate
                ...
                instruction_list.append(_cir.Gate('CNOT',[j,upperl_ones[0]]))
                instruction_list.append(_cir.Gate('CNOT',[upperl_ones[0],j]))
                instruction_list.append(_cir.Gate('CNOT',[j,upperl_ones[0]]))
            ...
            # Finally, try using swap and Hadamard gates.
                ...
                # THIS DOESN'T CURRENTLY FIND THE CLOSEST QUBIT TO SWAP WITH
                instruction_list.append(_cir.Gate('H',lowerl_ones[0]))
                M_updated = _symp.symplectic_action(M_updated, 'H', [lowerl_ones[0],])
                instruction_list.append(_cir.Gate('CNOT',[j,lowerl_ones[0]]))
                instruction_list.append(_cir.Gate('CNOT',[lowerl_ones[0],j]))
                instruction_list.append(_cir.Gate('CNOT',[j,lowerl_ones[0]]))
        ...
        while len(upperl_ones) >= 2: 
            ...
            for i in range(0,num_pairs):
                ...
                instruction_list.append(_cir.Gate('CNOT',(controlq,targetq)))                   
        ...
        if j in lowerl_ones:
            instruction_list.append(_cir.Gate('P',j))
    ...
    circuit = _cir.Circuit(gate_list=instruction_list,n=d)
...
#
# Todo : remove this function, and put its functionality inside the compile clifford from device spec function
#
                ...
                while not swap_quit:
                    ...
                    if j >= d:
                        ...
                        if matrix[j,i] == 1:
                            ...
                            instruction_list.append(_cir.Gate('CNOT',(i,j)))
                            instruction_list.append(_cir.Gate('CNOT',(j,i)))
                            instruction_list.append(_cir.Gate('CNOT',(i,j)))
                            swap_quit = True
                        else:
                            j += 1
                ...
                while not swap_quit:
                    ...
                    if j >= d:
                        ...
                        if matrix[i,j] == 1:
                            ...
                            matrix[:,i] = jcol
                            matrix[:,j] = icol
                            instruction_list.insert(0,_cir.Gate('CNOT',(i,j)))
                            instruction_list.insert(0,_cir.Gate('CNOT',(j,i)))
                            instruction_list.insert(0,_cir.Gate('CNOT',(i,j)))
            ...
            for j in range(0,k):
                ...
                if optype == 'row':
                    ...
                    if matrix[l,k] == 1:
                        ...
                        if position == 'upper':
                            instruction_list.append(_cir.Gate('CNOT',(k,l)))
                        if position == 'lower':
                            instruction_list.append(_cir.Gate('CNOT',(l,k)))
                        if paired_matrix:
                ...
                if optype == 'column':
                    ...
                    if matrix[k,l] == 1:
                        ...
                        instruction_list.insert(0,_cir.Gate('CNOT',(k,l)))
                        if paired_matrix:
...
def stabilizer_measurement_preparation_circuit(s,p,ds,iterations=1):
    ...
    for q in range(0,n):
        if vec[q] == 0 and vec[q+n] == 0:
            pauli_layer.append(_cir.Gate('I',q))
        elif vec[q] == 0 and vec[q+n] == 1:
            pauli_layer.append(_cir.Gate('Z',q))
        elif vec[q] == 1 and vec[q+n] == 0:
            pauli_layer.append(_cir.Gate('X',q))
        elif vec[q] == 1 and vec[q+n] == 1:
            pauli_layer.append(_cir.Gate('Y',q))
...
def symplectic_as_conditional_clifford_circuit_over_CHP(s,ds=None,returnall=False):
    ...
    def stage1(s):
        ...
        instructions = [_cir.Gate('H',i) for i in h_list]
    ...
    def stage3(s):
        ...
        for i in range(0,n):
            ...
            if matrix[i,i] != 1:
                matrix[i,i] = 1
                instructions.append(_cir.Gate('P',i))
    ...
    def stage4(s):
        ...
        circuit = _cir.Circuit(gate_list=instructions,n=n)
    ...
    def stage6(s):
        ...
        instructions = [_cir.Gate('P',i) for i in range(0,n)]
    ...
    def stage7(s):
        ...
        instructions = [_cir.Gate('H',i) for i in range(0,n)]
