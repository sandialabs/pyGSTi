...
from StdInputParser import StdInputParser as _StdInputParser
...
def writeEmptyDatasetFile(filename, gateStringList, 
    ...
    if len(gateStringList) > 0 and not isinstance(gateStringList[0], _gatestring.GateString):
...
def writeGateset(gs,filename,title=None):
    ...
    for (label,gate) in gs.iteritems():
        print >> f, label
        print >> f, "PauliMx"
        print >> f, _MOps.mxToString(gate, width=16, prec=8)
        print >> f, ""
