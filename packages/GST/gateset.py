...
import LatexUtil as _LatexUtil
...
class GateSet(_collections.OrderedDict):
    ...
    def Pr(self, spamLabel, gatestring, clipTo=None, bUseScaling=True):
        ...
        if bUseScaling:
            ...
            p = _MOps.trace( _np.dot(self.SPAMs[spamLabel],G) ) * scale # probability, with scaling applied (may generate overflow, but OK)
            ...
            #    try:
            #        test = _MOps.trace( _np.dot(self.SPAMs[spamLabel],G) ) * scale
            ...
            p = _MOps.trace( _np.dot(self.SPAMs[spamLabel],G) )
        ...
        if _np.isnan(p): 
            ...
            #
            #    p = _MOps.trace( _np.dot(self.SPAMs[spamLabel],G) ) * exp(total_exp) # probability
    ...
    def dPr(self, spamLabel, gatestring,
        ...
        if returnPr:
            p = _MOps.trace(_np.dot(self.SPAMs[spamLabel],prod)) * scale  #may generate overflow, but OK
...
#                scaleCache[i] = scaleCache[iLeft] + scaleCache[iRight] + log(nG)
    ...
    def diff_Frobenius(self, otherGateSet, transformMx=None, gateWeight=1.0, spamWeight=1.0, normalize=True):
        ...
        if T is not None:
            ...
            for gateLabel in self:
                d += gateWeight * _MOps.frobeniusNorm2( _np.dot(Ti,_np.dot(self[gateLabel],T)) - otherGateSet[gateLabel] )
                nSummands += gateWeight * _np.size(self[gateLabel])

            for (i,rhoV) in enumerate(self.rhoVecs): 
                d += spamWeight * _MOps.frobeniusNorm2(_np.dot(Ti,rhoV)-otherGateSet.rhoVecs[i])
                nSummands += spamWeight * _np.size(rhoV)

            for (i,Evec) in enumerate(self.EVecs):
                d += spamWeight * _MOps.frobeniusNorm2(_np.dot(_np.transpose(T),Evec)-otherGateSet.EVecs[i])
                nSummands += spamWeight * _np.size(Evec)

            if self.identityVec is not None:
                d += spamWeight * _MOps.frobeniusNorm2(_np.dot(_np.transpose(T),self.identityVec)-otherGateSet.identityVec)
                nSummands += spamWeight * _np.size(self.identityVec)


            #for (spamLabel,spamGate) in self.SPAMs.iteritems():
            #    if spamGate is not None:
            #        d += _MOps.frobeniusNorm( _np.dot(Ti,_np.dot(spamGate,T)) - otherGateSet.SPAMs[spamLabel] )
        else:
            for gateLabel in self:
                d += gateWeight * _MOps.frobeniusNorm2(self[gateLabel]-otherGateSet[gateLabel])
                nSummands += gateWeight * _np.size(self[gateLabel])

            for (i,rhoV) in enumerate(self.rhoVecs): 
                d += spamWeight * _MOps.frobeniusNorm2(rhoV-otherGateSet.rhoVecs[i])
                nSummands += spamWeight *  _np.size(rhoV)

            for (i,Evec) in enumerate(self.EVecs):
                d += spamWeight * _MOps.frobeniusNorm2(Evec-otherGateSet.EVecs[i])
                nSummands += spamWeight * _np.size(Evec)

            if self.identityVec is not None:
                d += spamWeight * _MOps.frobeniusNorm2(self.identityVec-otherGateSet.identityVec)
                nSummands += spamWeight * _np.size(self.identityVec)

            #for (spamLabel,spamGate) in self.SPAMs.iteritems():
            #    if spamGate is not None:
            #        d += _MOps.frobeniusNorm(spamGate - otherGateSet.SPAMs[spamLabel] )
    ...
    def diff_JTraceDistance(self, otherGateSet, transformMx=None):
        ...
        if T is not None:
            Ti = _nla.inv(T)
            dists = [ _JOps.JTraceDistance( _np.dot(Ti,_np.dot(self[gateLabel],T)), otherGateSet[gateLabel] ) for gateLabel in self ]
            for (spamLabel,spamGate) in self.SPAMs.iteritems():
                if spamGate is not None:
                    dists.append( _JOps.JTraceDistance( _np.dot(Ti,_np.dot(spamGate,T)), otherGateSet.SPAMs[spamLabel] ) )
        else:
            dists = [ _JOps.JTraceDistance(self[gateLabel], otherGateSet[gateLabel]) for gateLabel in self ]
            for (spamLabel,spamGate) in self.SPAMs.iteritems():
                if spamGate is not None:
                    dists.append( _JOps.JTraceDistance(spamGate, otherGateSet.SPAMs[spamLabel] ) )
        return max(dists)
    ...
    def __str__(self):
        s = ""
        for (i,rhoVec) in enumerate(self.rhoVecs):
            s += "rhoVec[%d] = " % i + _MOps.mxToString(_np.transpose(rhoVec)) + "\n"
        s += "\n"
        for (i,EVec) in enumerate(self.EVecs):
            s += "EVec[%d] = " % i + _MOps.mxToString(_np.transpose(EVec)) + "\n"
        s += "\n"
        for (l,gatemx) in self.iteritems():
            s += l + " = \n" + _MOps.mxToString(gatemx) + "\n\n"
        return s

#Use LatexUtil module now
#    def latex(self, gateLabel):
#        """ 
#        Return a latex string for a given label
#
#        Parameters
#        ----------
#        gateLabel : string
#            a gate label, "rho_<index>" or "E_<index>"
#
#        Returns
#        -------
#        string
#            latex code for typesetting the string for this item.
#        """
#        if gateLabel in self:
#            gate = self[gateLabel]
#        elif gateLabel.startswith("rho"): # e.g. rho, rho_0, rho_1
#            parts = gateLabel.split("_")
#            indx = int(parts[1]) if len(parts) == 2 else 0
#            gate = self.rhoVecs[indx]
#        elif gateLabel.startswith("E"): # e.g. E, E_0, E_1
#            parts = gateLabel.split("_")
#            indx = int(parts[1]) if len(parts) == 2 else 0
#            gate = self.EVecs[indx]
#        else: raise ValueError("GateSet cannot render '%s' in latex" % gateLabel)
#        return _LatexUtil.latex_matrix(gate)
