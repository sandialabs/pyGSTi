...
import GateStringTools as _GSTools
...
class DataSet:
  ...
  def __getstate__(self):
    toPickle = { 'gsIndexKeys': map(_GSTools.compressGateLabelTuple, self.gsIndex.keys()),
  ...
  def __setstate__(self, state_dict):
    self.gsIndex = _OrderedDict( zip( map(_GSTools.expandGateLabelTuple, state_dict['gsIndexKeys']), state_dict['gsIndexVals']) )
  ...
  def save(self, fileOrFilename):
    ...
    toPickle = { 'gsIndexKeys': map(_GSTools.compressGateLabelTuple, self.gsIndex.keys() if self.gsIndex else []),
