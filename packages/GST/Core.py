...
import GateSetTools as _GSTools
import Exceptions as _E
import gateset as _gateset
import spamspec as _spamspec
import Gate as _Gate
import gatestring as _GS
import GateStringTools as _GateStringTools
...
# 3) use Jamiol. Iso in printGatesetInfo(...)
...
def doLGST(dataset, specs, targetGateset=None, gateLabels=None, gateLabelAliases={},
  ...
  else: raise _E.GSTValueError("doLGST cannot determine gate labels to estimate from supplied parameters")

  if spamDict is None:
    if targetGateset is not None:
      spamDict = targetGateset.get_SPAM_label_dict()
    else: raise _E.GSTValueError("doLGST cannot determine SPAM dictionary from supplied parameters")
  ...
  if identityVec is None: #check again in case targetGateset.identityVec == None
    for (ir,ie) in spamDict.keys():
      if ie == -1 and ir != -1: #then identityVec is required b/c this spamlabel represents Evec = identityVec - sum(other_Evecs)
        raise _E.GSTValueError("doLGST cannot determine the identity vector from supplied parameters")
  ...
  for gateLabel in gateLabelsToEstimate:
    ...
    lgstGateset.set_gate(gateLabel, _Gate.FullyParameterizedGate(_np.dot(invABMat_p,X_p))) # shape (trunc,trunc)
    #print "DEBUG: X(%s) = \n" % gateLabel,X
  ...
  if identityVec is not None:
    ...
    lgstGateset.set_identityVec( padded_identityVec )
  ...
  if guessGatesetForGauge is not None:  
    ...
    for gateLabel in gateLabelsToEstimate:
      if gateLabel in guessGatesetForGauge:
        ...
        _Gate.optimizeGate( new_gate, lgstGateset.get_gate(gateLabel), bG0=True )
...
def _constructXMatrix(rhoSpecs, ESpecs, spamDict, gateLabelTuple, dataset):
  ...
  for i,espec in enumerate(ESpecs):
    for j,rhospec in enumerate(rhoSpecs):
      gateLabelString = rhospec.str + _GS.GateString(gateLabelTuple) + espec.str # LEXICOGRAPHICAL VS MATRIX ORDER
      spamLabel = spamDict[ (rhospec.i,espec.i) ]
      try:
        dsRow = dataset[gateLabelString]
      except:
...
def gramRankAndEvals(dataset, specs, targetGateset=None, spamDict=None):
  """
...
def listStringsLGSTcanEstimate(dataset, specs):
  ...
  return _GateStringTools.gateStringList(estimatable)
...
def doExLGST(dataset, startGateset, gateStringsToUseInEstimation, specs, 
  ...
  if len(gateStringsToUseInEstimation) > 0 and isinstance(gateStringsToUseInEstimation[0],_GS.GateString):
  ...
  lgstEstimates = doLGST(dataset, specs, targetGateset, gateLabelAliases.keys(),
  ...
  if verbosity <= 2:
    ...
    if regularizeFactor == 0:
      def objectiveFunc(vectorGS):
        gs.fromVector(vectorGS,G0=opt_G0, SP0=opt_SP0, SPAM=opt_SPAM, gates=opt_gates)    
  ...
  if verbosity <= 3:
    if regularizeFactor == 0:
      def jacobian(vectorGS):
        ...
        if check_jacobian: _OPT.check_jac(objectiveFunc, vectorGS, jac, tol=1e-3, eps=1e-6, errType='abs')
        return jac
    else:
      def jacobian(vectorGS):
        gs.fromVector(vectorGS,G0=opt_G0, SP0=opt_SP0, SPAM=opt_SPAM, gates=opt_gates)
    ...
    def jacobian(vectorGS):
      ...
      if check_jacobian:
        errSum, errs, fd_jac = _OPT.check_jac(objectiveFunc, vectorGS, jac, tol=1e-3, eps=1e-6, errType='abs')
  ...
  #  def obj_i(x, i): return objectiveFunc(x)[i]
  #  def jac_i(x, i): return (jacobian(x))[i]
  #  y = objectiveFunc(vectorGS)
  ...
  x0 = gs.toVector(G0=opt_G0, SP0=opt_SP0, SPAM=opt_SPAM, gates=opt_gates)
  opt_x, opt_jac, info, msg, flag = \
      _spo.leastsq( objectiveFunc, x0, xtol=tol, ftol=tol, gtol=tol,
               maxfev=maxfev*(len(x0)+1), full_output=True, Dfun=jacobian)
  full_minErrVec = objectiveFunc(opt_x)
  ...
  #minErrVec_chk = objectiveFunc(opt_x_chk)

  gs.fromVector(opt_x,G0=opt_G0, SP0=opt_SP0, SPAM=opt_SPAM, gates=opt_gates)
  ...
  if verbosity > 1:
      ...
      #try: print "   log(likelihood) = ", _LF.logL(gs, dataset)
      #except: pass
...
def doIterativeExLGST(dataset, startGateset, specs, gateStringSetsToUseInEstimation, 
                      targetGateset=None, spamDict=None, guessGatesetForGauge=None,
                      svdTruncateTo=0, maxiter=100000, maxfev=None, tol=1e-6, 
...
#          'ignore' - skip string silently and proceed
  ...
  if len(gateStringSetsToUseInEstimation ) > 0 and \
     ...
     isinstance(gateStringSetsToUseInEstimation[0][0],_GS.GateString):
  ...
  for (i,stringsToEstimate) in enumerate(gateStringLists):
    ...
    minErr, elgstGateset = doExLGST( dataset, elgstGateset, stringsToEstimate, specs,
...
def doLSGST(dataset, startGateset, gateStringsToUse, 
  ...
  #convert list of GateStrings to list of raw tuples since that's all we'll need
  if len(gateStringsToUse) > 0 and isinstance(gateStringsToUse[0],_GS.GateString):
  ...
  vec_gs_len = len(gs.toVector(G0=opt_G0, SP0=opt_SP0, SPAM=opt_SPAM, gates=opt_gates))
  ...
  if maxEvalSubTreeSize is not None:
    ...
    if verbosity > 2: 
      ...
      evTree.printAnalysis()

  if useFreqWeightedChiSq:
    def getWeights(p):
      return fweights
    def getDWeights(p,wts):
      return z
  else:
    def getWeights(p):
    ...
    def getDWeights(p,wts):  #derivative of weights w.r.t. p
      cp = _np.clip(p,minProbClipForWeighting,1-minProbClipForWeighting)
  ...
  if cptp_penalty_factor == 0:
    ...
    if verbosity <= 2:  # Fast versions of functions
      if regularizeFactor == 0:
        def objectiveFunc(vectorGS):
          gs.fromVector(vectorGS,G0=opt_G0, SP0=opt_SP0, SPAM=opt_SPAM, gates=opt_gates)
          gs.Bulk_fillProbs(probs, spam_lbl_rows, evTree, probClipInterval, check) 
          v = (probs-f)*getWeights(probs) # dims K x M (K = nSpamLabels, M = nGateStrings)
          return v.reshape([KM])
    ...
    if verbosity <= 3: # Fast versions of functions
      if regularizeFactor == 0: # Fast un-regularized version
        def jacobian(vectorGS):
          gs.fromVector(vectorGS,G0=opt_G0, SP0=opt_SP0, SPAM=opt_SPAM, gates=opt_gates)
          gs.Bulk_filldProbs(dprobs, spam_lbl_rows, evTree, 
                             G0=opt_G0, SP0=opt_SP0, SPAM=opt_SPAM, gates=opt_gates,
                             prMxToFill=probs, clipTo=probClipInterval, check=check)
          weights  = getWeights( probs )
          #jac = dPr * (weights+(Pr-f)*getDWeights( Pr, weights ))[:,None] #OLD  # (M,N) * (M,1) = (M,N)
          jac = dprobs * (weights+(probs-f)*getDWeights(probs, weights ))[:,:,None]  # (K,M,N) * (K,M,1)   (N = dim of vectorized gateset)
      ...
      def jacobian(vectorGS):
        ...
        dPr_prefactor = (weights+(probs-f)*getDWeights( probs, weights )) # (K,M)
    ...
    raise ValueError("CPTP-penalized LSGST not implemented yet.")
    #def objectiveFunc(vectorGS):  #TODO: Upgrade from 'plus' restricted case
    #  gs.fromVector(vectorGS,G0=opt_G0, SP0=opt_SP0, SPAM=opt_SPAM, gates=opt_gates)
    #  p = gs.Bulk_Pr('plus', evTree, clipTo=probClipInterval, check=check) #RESTRICTION: 'plus' assumes only a single 'plus' spam label
    #  #p = _np.array( [ gs.Pr('plus',gateStr) for gateStr in gateStringsToUse ] ) #OLD
    #  negEvals = cptp_penalty_factor * _np.array(_JOps.magsOfNegativeJEvals(gs))
    #  rhoPenalties = cptp_penalty_factor * _np.array([ _LF.rhoVecPenalty(r) for r in gs.rhoVecs ])
    #  EPenalties   = cptp_penalty_factor * _np.array([ _LF.EVecPenalty(e) for e in gs.EVecs ])
    #
    #  if regularizeFactor > 0:
    #    gsVecNorm = regularizeFactor * _np.array( [ max(0,absx-1.0) for absx in map(abs,vectorGS) ], 'd')
    #    return _np.concatenate( ((p - f) * getWeights(p), negEvals, rhoPenalties, EPenalties, gsVecNorm) )
    #  else: return _np.concatenate( ((p - f) * getWeights(p), negEvals, rhoPenalties, EPenalties) )
    #
    #jacobian = None
  ...
  opt_x, opt_jac, info, msg, flag = \
      _spo.leastsq( objectiveFunc, x0, xtol=tol, ftol=tol, gtol=tol,
               maxfev=maxfev*(len(x0)+1), full_output=True, Dfun=jacobian ) 
  full_minErrVec = objectiveFunc(opt_x)  #note: calls gs.fromVector(opt_x,...) so don't need to call this again
  minErrVec = full_minErrVec if regularizeFactor == 0 else full_minErrVec[0:-len(x0)] #don't include regularization terms
  soln_gs = gs.copy();
  ...
  if verbosity > 1:
    ...
    nModelParams = gs.getNumNonGaugeParams(opt_gates, opt_G0, opt_SPAM, opt_SP0) #len(x0)
    nDataParams  = nGateStrings*(len(dataset.getSpamLabels())-1) #number of independent parameters
  ...
  #if targetGateset is not None:
  #  target_vec = targetGateset.toVector(G0=opt_G0, SP0=opt_SP0, SPAM=opt_SPAM, gates=opt_gates)
  #  targetErrVec = objectiveFunc(target_vec)
  #  return minErrVec, soln_gs, targetErrVec
  return minErrVec, soln_gs

def doLSGSTwithModelSelection(dataset, startGateset, dimDelta, gateStringsToUse, 
                              maxiter=100000, maxfev=None, tol=1e-6, 
                              opt_gates=True, opt_G0=True, opt_SPAM=True, opt_SP0=True,
                              cptp_penalty_factor=0, minProbClipForWeighting=1e-4, probClipInterval=None,
                              useFreqWeightedChiSq=False, regularizeFactor=0, verbosity=0,
  ...
  #Run doLSGST multiple times - one for the starting Gateset and one for the starting gateset
  ...
  minErr, gs = doLSGST(dataset, startGateset, gateStringsToUse, maxiter, maxfev, tol, 
  ...
  nParamsBest = len(startGateset.toVector(G0=opt_G0, SP0=opt_SP0, SPAM=opt_SPAM, gates=opt_gates))
  origGS = bestGS = gs
  bestMinErr = minErr
  ...
  while tryDecreasedDim:
    curDim -= dimDelta
    curStartGateset = _GSTools.decreaseGatesetDimension(curStartGateset, curDim)
    nParams = len(curStartGateset.toVector(G0=opt_G0, SP0=opt_SP0, SPAM=opt_SPAM, gates=opt_gates))

    minErr, gs = doLSGST(dataset, curStartGateset, gateStringsToUse, maxiter, maxfev, tol, 
                       opt_gates, opt_G0, opt_SPAM, opt_SP0, cptp_penalty_factor, 
                       minProbClipForWeighting, probClipInterval,
                       useFreqWeightedChiSq, regularizeFactor, verbosity,
                       check, check_jacobian, gatestringWeights, None, memLimit)
  ...
  while tryIncreasedDim:
    curDim += dimDelta
    curStartGateset = _GSTools.increaseGatesetDimension(curStartGateset, curDim)
    curStartGateset = _GSTools.randomKickGateset(curStartGateset, 0.01) #give random kick here??
    nParams = len(curStartGateset.toVector(G0=opt_G0, SP0=opt_SP0, SPAM=opt_SPAM, gates=opt_gates))
    if nParams > nStrings: 
...
def doIterativeLSGST(dataset, startGateset, gateStringSetsToUseInEstimation, 
                     maxiter=100000, maxfev=None, tol=1e-6, 
                     opt_gates=True, opt_G0=True, opt_SPAM=True, opt_SP0=True,
                     cptp_penalty_factor=0, minProbClipForWeighting=1e-4, probClipInterval=None,
  ...
  for (i,stringsToEstimate) in enumerate(gateStringLists):
    ...
    minErr, lsgstGateset = doLSGST( dataset, lsgstGateset, stringsToEstimate,
...
def doIterativeLSGSTwithModelSelection(dataset, startGateset, dimDelta, gateStringSetsToUseInEstimation, 
                                       maxiter=100000, maxfev=None, tol=1e-6, 
                                       opt_gates=True, opt_G0=True, opt_SPAM=True, opt_SP0=True,
                                       cptp_penalty_factor=0, minProbClipForWeighting=1e-4, probClipInterval=None,
                                       useFreqWeightedChiSq=False, regularizeFactor=0, returnErrorVec=False, returnAll=False,
  ...
  for (i,stringsToEstimate) in enumerate(gateStringLists):
    ...
    minErr, lsgstGateset = doLSGSTwithModelSelection( dataset, lsgstGateset, dimDelta, stringsToEstimate,
                                    maxiter, maxfev, tol, opt_gates, opt_G0, opt_SPAM, opt_SP0,
                                    cptp_penalty_factor, minProbClipForWeighting, probClipInterval,
                                    useFreqWeightedChiSq, regularizeFactor, verbosity, check, check_jacobian,
...
def doMLEGST(dataset, startGateset, gateStringsToUse, 
             maxiter=100000, maxfev=None, tol=1e-6,
             opt_gates=True, opt_G0=True, opt_SPAM=True, opt_SP0=True,
    ...
    spamLabels = gs.get_SPAM_labels() #this list fixes the ordering of the spam labels
    vec_gs_len = gs.getNumParams(opt_gates,opt_G0,opt_SPAM,opt_SP0)
    ...
    _LF.fillCountVecs(cntVecMx, spam_lbl_rows, dataset, dsGateStringsToUse)
    logL_upperbound = _LF.logL_max(dataset, dsGateStringsToUse, cntVecMx, poissonPicture) # The theoretical upper bound on the log(likelihood)
    ...
    x0 = gs.toVector(G0=opt_G0, SP0=opt_SP0, SPAM=opt_SPAM, gates=opt_gates)
    opt_x, opt_jac, info, msg, flag = \
        _spo.leastsq( objectiveFunc, x0, xtol=tol, ftol=tol, gtol=0,
                      maxfev=maxfev*(len(x0)+1), full_output=True, Dfun=jacobian ) 
    ...
    minErrVec = objectiveFunc(opt_x)  #note: calls gs.fromVector(opt_x,...) so don't need to call this again
    ...
    if verbosity > 1:
      if _np.isfinite(deltaLogL):
        ...
        #print " DEBUG LOGL = ", _LF.logL(gs, dataset, gateStringsToUse),
        #  " DELTA = ",(logL_upperbound-_LF.logL(gs, dataset, gateStringsToUse))
      else:
...
def doIterativeMLEGST(dataset, startGateset, gateStringSetsToUseInEstimation,
                      maxiter=100000, maxfev=None, tol=1e-6,
                      opt_gates=True, opt_G0=True, opt_SPAM=True, opt_SP0=True,
                      minProbClip=1e-4, probClipInterval=None, radius=1e-4, poissonPicture=True,
  ...
  for (i,stringsToEstimate) in enumerate(gateStringLists):
    ...
    chi2Diff, mleGateset = doLSGST( dataset, mleGateset, stringsToEstimate,
                                    maxiter, maxfev, tol, opt_gates, opt_G0, opt_SPAM, opt_SP0,
                                    0, minProbClip, probClipInterval, False, 0, verbosity, check,
                                    False, None, None, memLimit) # so maxLogL is really chi2 number here

    logL_ub = _LF.logL_max(dataset, stringsToEstimate, None, poissonPicture, check)
    maxLogL = _LF.logL(mleGateset, dataset, stringsToEstimate, minProbClip, probClipInterval,
    ...
    #maxLogL, mleGateset = doMLEGST( dataset, mleGateset, stringsToEstimate,
    #                                maxiter, maxfev, tol, opt_gates, opt_G0, opt_SPAM, opt_SP0,
    ...
    if i == len(gateStringLists)-1: #on the last iteration, do MLE
      ...
      maxLogL_p, mleGateset_p = doMLEGST( dataset, mleGateset, stringsToEstimate,
                                          maxiter, maxfev, tol, opt_gates, opt_G0, opt_SPAM, opt_SP0,
                                          minProbClip, probClipInterval, radius, poissonPicture,
                                          verbosity, check, None, memLimit)
      if verbosity > 0:
      ...
      if maxLogL_p > maxLogL: #if doMLEGST improved the maximum log-likelihood
        maxLogL = maxLogL_p
...
def printGatesetInfo(gateset):
    ...
    for (label,gate) in gateset.iteritems():
        print "Choi(%s) in pauli basis = \n" % label, _MOps.complexMxToString(_JOps.opWithJamiolkowskiIsomorphism(gate))
        print "  --eigenvals = ", sorted( [ev.real for ev in _np.linalg.eigvals(_JOps.opWithJamiolkowskiIsomorphism(gate))] ),"\n"
    print "Sum of negative Choi eigenvalues = ", _JOps.sumOfNegativeJEvals(gateset)
...
#Note: this code overlaps doMLEGST a lot -- consolidate in FUTURE?
def optimizeGauge(gateset, toGetTo, maxiter=100000, maxfev=None, tol=1e-8,
    ...
    if toGetTo == "target":
      ...
      def objectiveFunc(vectorM):
        ...
        if cpPenalty != 0:
          s = _JOps.sumOfNegativeJEvals(gs)
          if s > 1e-8: return cpPenalty #*(1.0+s) #1e-8 should match TOL in contract to CP routines

        if spamPenalty != 0:
          sp =  sum( [ _LF.rhoVecPenalty(rhoVec) for rhoVec in gs.rhoVecs ] )
          sp += sum( [ _LF.EVecPenalty(EVec)     for EVec   in gs.EVecs ] )
        ...
        if targetGatesMetric == "frobenius":
          ...
          for gateLbl in gs:
            diff += gateWeight * (1.0 - _JOps.ProcessFidelity(targetGateset[gateLbl], gs[gateLbl]))
        elif targetGatesMetric == "tracedist":
          for gateLbl in gs:
            diff += gateWeight * _JOps.JTraceDistance(targetGateset[gateLbl], gs[gateLbl])
        ...
        if targetSpamMetric == "frobenius":
          ...
          for spamlabel in gs.SPAMs.keys(): 
            diff += spamWeight * (1.0 - _JOps.ProcessFidelity(targetGateset.SPAMs[spamlabel], gs.SPAMs[spamlabel]))
        elif targetSpamMetric == "tracedist":
          for spamlabel in gs.SPAMs.keys(): 
            diff += spamWeight * _JOps.JTraceDistance(targetGateset.SPAMs[spamlabel], gs.SPAMs[spamlabel])
      ...
      if constrainToTP: #assume gateset is already in TP so no TP optimization needed
        ...
        minf, tpGaugeMx, tpGateset = optimizeGauge(gateset,"TP", maxiter, maxfev, tol,
      ...
      def objectiveFunc(vectorM):
        ...
        cpPenalties = _JOps.sumsOfNegativeJEvals(gs)
        ...
        spamPenalty =  sum( [ _LF.rhoVecPenalty(rhoVec) for rhoVec in gs.rhoVecs ] )
        spamPenalty += sum( [ _LF.EVecPenalty(EVec)     for EVec   in gs.EVecs ] )
      ...
      if constrainToTP: #assume gateset is already in TP so no TP optimization needed
        ...
        minf, tpGaugeMx, tpGateset = optimizeGauge(gateset, "TP and target", maxiter, maxfev, tol,
                                                   'L-BFGS-B', targetGateset, targetFactor, 
                                                   constrainToTP, constrainToCP, constrainToValidSpam, True,
                                                   gateWeight, spamWeight, verbosity)
      ...
      def objectiveFunc(vectorM):
        ...
        for gateLabel in gs:
          d += _MOps.frobeniusNorm(gs[gateLabel]-complDepolGate)
        spamPenalty  = sum( [ _LF.rhoVecPenalty(rhoVec) for rhoVec in gs.rhoVecs ] )
        spamPenalty += sum( [ _LF.EVecPenalty(EVec)     for EVec   in gs.EVecs ] )        
        return d + spamPenalty

    else: raise ValueError("Invalid toGetTo passed to optimizeGauge: %s" % toGetTo)
    ...
    minSol = _OPT.minimize(objectiveFunc, x0,
                          ...
                          callback = printObjFunc if verbosity > 2 else None) #stopval=1e-7 -- (before I added log10)
    ...
    if verbosity > 1:
      if toGetTo == "target":
        ...
        for gateLabel in newGateset:
          print "  frobenius norm diff of %s = %g" % (gateLabel, _MOps.frobeniusNorm(newGateset[gateLabel]-targetGateset[gateLabel]))
        for (i,rhoV) in enumerate(newGateset.rhoVecs): 
          print "  frobenius norm diff of rho[%d] = %g" % (i, _MOps.frobeniusNorm(rhoV-targetGateset.rhoVecs[i]))
        for (i,Evec) in enumerate(newGateset.EVecs): 
          print "  frobenius norm diff of EVec[%d] = %g" % (i, _MOps.frobeniusNorm(Evec-targetGateset.EVecs[i]))
      else:
...
#def optimizeUnitaryGauge(gateset,dataset,verbosity,**kwargs):
...
#
#    def objectiveFunc(v):
#      matM = singleQubitGate( v[0], v[1], v[2] )
#      gs = gateset.copy()
#      gs.transform(matM)
#      ret = -_np.linalg.norm(_LF.dlogL(gs, dataset, **kwargs))
#      print "DEBUG: ",ret
#      return ret
#
#    startV = _np.array( [0,0,0] )
#    printObjFunc = _OPT.createObjFuncPrinter(objectiveFunc)
#    minSol = _OPT.minimize(objectiveFunc,startV,method=method, tol=tol,
#                          callback = printObjFunc if verbosity > 2 else None)
#
#    gaugeMat = singleQubitGate( minSol.x[0], minSol.x[1], minSol.x[2] )
#    newGateset = gateset.copy()
#    newGateset.transform(gaugeMat)
...
def _contractToXP(gateset,dataset,verbosity,method='Nelder-Mead',
    ...
    def objectiveFunc(vectorGS):
        gs.fromVector(vectorGS,SPAM=False,G0=opt_G0)
        forbiddenProbPenalty = _LF.forbiddenProb(gs,dataset)
        return (CLIFF + forbiddenProbPenalty if forbiddenProbPenalty > 1e-10 else 0) + gs.diff_Frobenius(gateset)

    printObjFunc = _OPT.createObjFuncPrinter(objectiveFunc)
    if objectiveFunc(gs.toVector(SPAM=False,G0=opt_G0)) < 1e-8:  
      if verbosity > 1: print 'Already in XP - no contraction necessary'
      return 0.0, gs

    optSol = _OPT.minimize(objectiveFunc,gs.toVector(SPAM=False,G0=opt_G0),
                          method=method, tol=tol, maxiter=maxiter,
                          callback = printObjFunc if verbosity > 2 else None)
                      
    gs.fromVector(optSol.x,SPAM=False,G0=opt_G0)
    gs.log("Contract to XP", { 'method': method, 'tol': tol, 'maxiter': maxiter, 'opt_G0': opt_G0 } )
    if optSol.fun >= CLIFF: raise _E.GSTRuntimeError("Failed to contractToXP")
...
def _contractToCP(gateset,verbosity,method='Nelder-Mead',
    ...
    def objectiveFunc(vectorGS):
        gs.fromVector(vectorGS,SPAM=False,G0=opt_G0)
        cpPenalty = _JOps.sumOfNegativeJEvals(gs) * 1000
    ...
    if optSol.fun >= CLIFF: raise _E.GSTRuntimeError("Failed to contractToCP")
...
def _contractToCP_direct(gateset,verbosity,TPalso=False,maxiter=100000,tol=1e-8):
    ...
    for (gateLabel,gate) in gateset.iteritems():
      ...
      Jmx = _JOps.opWithJamiolkowskiIsomorphism(new_gate)
      ...
      if abs( sum(evals) - 1.0 ) >= 1e-8: #DEBUG
        ...
        print "WARNING: JMx from: "; _MOps.printMx(new_gate)
...
#          assert( abs( sum(new_evals) - 1.0 ) < 1e-8 )
        ...
        assert( min(evals) >= -EVAL_TOL and abs( sum(evals) - 1.0 ) < 1e-8) #Check that trace-trunc above didn't mess up positivity

        new_gate = _JOps.opWithInvJamiolkowskiIsomorphism(new_Jmx)
        ...
        #
        #  new_Jmx = _JOps.opWithJamiolkowskiIsomorphism(new_gate)
        #  evals,evecs = _np.linalg.eig(new_Jmx)
      ...
      gs.set_gate(gateLabel, _Gate.FullyParameterizedGate( new_gate ) )

      if it > maxiter:
        print "Warning: Max iterations exceeded in contractToCP_direct"
      #else: print "contractToCP_direct success in %d iterations" % it  #DEBUG

      if verbosity > 2:
        print "Direct CP contraction of %s gate gives frobenius diff of %g" % \
            (gateLabel, _MOps.frobeniusNorm(gs[gateLabel] - gate))
...
def _contractToValidSPAM(gateset, verbosity=0):
    ...
    firstElTrace = _np.real( _MOps.trace(_BT.pauliProdVectorToMatrixInStdBasis(dummyVec)))  # == sqrt(2)**nQubits
    diff = 0
    ...
    for i in range(len(gs.rhoVecs)):
      ...
      mx = _BT.pauliProdVectorToMatrixInStdBasis(vec)
      ...
      while(lowEval < -TOL):
        ...
        lowEval = min( [ev.real for ev in _np.linalg.eigvals( _BT.pauliProdVectorToMatrixInStdBasis(vec) ) ])
    ...
    for i in range(len(gs.EVecs)):
      evals,evecs = _np.linalg.eig( _BT.pauliProdVectorToMatrixInStdBasis(gs.EVecs[i]) )
      if(min(evals) < 0.0 or max(evals) > 1.0):
        ...
        vec = _BT.matrixInStdBasisToPauliProdVector(mx)
...
def getRhoAndESpecs(fiducialGateStrings=None, rhoStrs=None, EStrs=None, rhoSpecs=None, ESpecs=None, rhoVecInds=(0,), EVecInds=(0,) ):
    ...
    if rhoSpecs is not None:
        ...
        rhoSpecs = [ _spamspec.SpamSpec(iRho,f) for f in rhoStrs for iRho in rhoVecInds ]
        if fiducialGateStrings is not None:
           raise ValueError("Can only specify one of rhoSpecs, rhoStrs, or fiducialGateStrings")
    elif fiducialGateStrings is not None:
        rhoSpecs = [ _spamspec.SpamSpec(iRho,f) for f in fiducialGateStrings for iRho in rhoVecInds ]
    else:
    ...
    if ESpecs is not None:
        ...
        ESpecs = [ _spamspec.SpamSpec(iEvec,f) for f in EStrs for iEvec in EVecInds ]
        if fiducialGateStrings is not None:
           raise ValueError("Can only specify one of ESpecs, EStrs, or fiducialGateStrings")
    elif fiducialGateStrings is not None:
        ESpecs = [ _spamspec.SpamSpec(iEvec,f) for f in fiducialGateStrings for iEvec in EVecInds ]
    else:
...
#def getRhoAndEStrs(**kwargs):
#    if (kwargs.has_key('rhoSpecs') or kwargs.has_key('rhoStrs')) and (kwargs.has_key('ESpecs') or kwargs.has_key('EStrs')):
#        if kwargs.has_key('rhoSpecs'):
#            rhoStrs = [ rhoSpec[:-1] for rhoSpec in kwargs['rhoSpecs'] ]
#        else:
#            rhoStrs = kwargs['rhoStrs'][:]
#
#        if kwargs.has_key('ESpecs'):
#            EStrs = [ ESpec[1:] for ESpec in kwargs['ESpecs'] ]
#        else:
#            EStrs = kwargs['EStrs'][:]
#
#    elif kwargs.has_key('fiducialGateStrings'):
#        rhoStrs = kwargs['fiducialGateStrings'][:]
#        EStrs =  kwargs['fiducialGateStrings'][:]
#    else:
#        raise ValueError("Cannot determine rho or E strings from supplied parameters.  " + \
#                         "You must specify either 'fiducialGateStrings' or (('rhoSpecs' or 'rhoStrs') and " +
#                         "('ESpecs' or 'EStrs')) as keyword arguments.")
#    return rhoStrs, EStrs
