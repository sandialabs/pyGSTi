...
def getSufficientFiducialPairs(targetGateset, rhoStrs, EStrs, germList, testLs=(256,2048), spamLabels="all", tol=0.75, verbosity=0, testPairList=None):
    ...
    def getDerivs(L):
        ...
        for iGerm,germ in enumerate(germList):
            ...
            lst = _GST.createGateStringList("f0+expGerm+f1", f0=rhoStrs, f1=EStrs,
    ...
    for nNeededPairs in range(1,nPossiblePairs):
        if verbosity > 0: 
            print "Beginning search for a good set of %d pairs (%d pair lists to test)" % (nNeededPairs,nCr(nPossiblePairs,nNeededPairs))
            _sys.stdout.flush()
...
def _old_TestPair(targetGateset, fiducialList, germList, L, testPairList, spamLabels="all"):
    ...
    dProbs = []
    for iGerm,germ in enumerate(germList):
        expGerm = _GST.GateStringTools.repeatWithMaxLength(germ,L)
