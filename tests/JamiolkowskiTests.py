...
class JamiolkowskiTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.testGate = GST.build_gate( self.stateSpaceDims, self.stateSpaceLabels, "LX(pi,0,2)","std") 
        self.testGateGM_mx = GST.BT.std_to_gm(self.testGate.matrix, self.stateSpaceDims)
        self.expTestGate_mx = GST.BT.expand_from_std_direct_sum_mx(self.testGate.matrix, self.stateSpaceDims)
        self.expTestGateGM_mx = GST.BT.std_to_gm(self.expTestGate_mx)

    def checkBasis(self, cmb):
        #Op with Jamio map on gate in std and gm bases
        Jmx1 = GST.JOps.jamiolkowski_iso(self.testGate.matrix, gateMxBasis='std',
                                                      choiMxBasis=cmb, dimOrStateSpaceDims=self.stateSpaceDims)
        Jmx2 = GST.JOps.jamiolkowski_iso(self.testGateGM_mx, gateMxBasis='gm',
                                                      choiMxBasis=cmb, dimOrStateSpaceDims=self.stateSpaceDims)
        ...
        JmxExp1 = GST.JOps.jamiolkowski_iso(self.expTestGate_mx,gateMxBasis='std',choiMxBasis=cmb)
        JmxExp2 = GST.JOps.jamiolkowski_iso(self.expTestGateGM_mx,gateMxBasis='gm',choiMxBasis=cmb)
        ...
        revTestGate_mx = GST.JOps.jamiolkowski_iso_inv(Jmx1,choiMxBasis=cmb,
        ...
        revExpTestGate_mx = GST.JOps.jamiolkowski_iso_inv(Jmx1,choiMxBasis=cmb,
                                                                      gateMxBasis='std')
        self.assertArraysAlmostEqual( GST.BT.contract_to_std_direct_sum_mx(revExpTestGate_mx,self.stateSpaceDims),
