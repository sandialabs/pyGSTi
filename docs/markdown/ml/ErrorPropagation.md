...
We construct a snipper using `layer_snipper_from_qubit_graph`, which describes the device's connectivity using a `networkx`/`igraph`/`graph-tool` graph, a pygsti `QubitGraph`/`QubitProcessorSpec`, or a raw adjacency matrix (see `pygsti.tools.graphs`):
