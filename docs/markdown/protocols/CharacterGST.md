...
| `tri_ramsey` | $\sqrt{Y} S$ | $\mathbb{Z}_3$ | complex | $\lambda_2^\triangle$, $\omega$ → angle-between-axes $\beta$ |
| `idle_s/y/tri` | germ · $G_i$ | — | complex | idle coherent components $\theta_{I,z}$, $\theta_{I,y}$, $\theta_{I,x}$ |
...
cGST's decay fits estimate the noisy germs' *eigenvalue deviations* — gauge-invariant quantities we can extract from the true model by eigendecomposition, with no first-order approximations:
...
* for the $S$ germ itself, $a = (1-\lambda_1)(1 - 2\tilde C)$ — opposite sign to the manuscript's formula, given its own channel convention $E_S[3,0] = -a$.
