...
```{note}
**This page is built without its output.** Executing it produces about 10 MB of inline
JavaScript, most of it hover text in the two color box plots, which is more than belongs
on a docs page. The code is complete and current, so run the notebook to see the figures:
use the launch buttons at the top of the page, or download it and run it locally.
```
