...
Generating a Binary RB experiment design is very similar to creating a Direct RB design: the inputs are the same, and the layer sampling matters for the same reason. What differs is the analysis, which is covered at the end of this page.
...
The structure of the circuit layers is specificed via the optional argument `layer_sampling`, which currently supports two layer structures. 
...
2. `alternating1q2q`: Each layer consists of two parts: first randomly sampled two-qubit gates, with all other qubits idling, then randomly sampled single-qubit gates on every qubit.
...
```{code-cell} ipython3
# The error rate we *approximately* expect according to Binary RB theory
