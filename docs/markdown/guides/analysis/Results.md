...
As a concrete example, we'll explore one of the `ModelEstimateResults` objects generated by the [GST protocols tutorial](../gst/RunningGST) (so **if you haven't run this tutorial, go do it now**).
...
if not os.path.isdir(data_dir + "/results/GateSetTomography"):
    # Not run the GST protocols tutorial? Produce the same results here so this
    ...
    pygsti.protocols.GateSetTomography(
        smq1Q_XYI.target_model("full TP"), verbosity=0).run(data).write()

results = pygsti.io.read_results_from_dir(data_dir, "GateSetTomography")
print(results)
```
