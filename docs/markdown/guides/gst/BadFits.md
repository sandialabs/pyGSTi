...
The same logic extends past parameterization to the structure of the model itself. If a gate behaves differently depending on what precedes it, you do not have one gate, you have two, and the fix is to model them as two: see [context-dependent GST](../../advanced/specialist/ContextDependence). If the device changes over the run, the honest options are to make the run shorter, interleave the circuit order so drift averages rather than correlates with depth, or move to a time-resolved analysis.

## Where to go next

If you have not yet checked stability, do that before anything else on this page: [drift characterization](../drift/DriftCharacterization) will tell you in minutes whether the answer is "your calibration moved". If you have ruled out drift and the violation tracks which gates precede which, [context-dependent GST](../../advanced/specialist/ContextDependence) shows how to split a label in two and let GST estimate both. If a small number of circuits are doing all the damage, [robust GST using TVD](../../advanced/specialist/RobustGST-TVD) is the right tool.
