...
Idle tomography has identified the errors we placed in `mdl_datagen`, but note the normalization caveat below before comparing the numbers directly.

```{warning}
**The intrinsic stochastic rates reported in this table are not on the same scale as the rates injected into `mdl_datagen` — they differ by a factor of 2.** This is an unresolved normalization convention in idle tomography's reporting path, not a property of your data. Do not read absolute stochastic error rates off this table without accounting for that factor.
```
...
Served with these docs: <a href="../../../reports/IDTTestReport/main.html">IDTTestReport</a>.
