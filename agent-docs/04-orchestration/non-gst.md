...
- **`su2rb.py`** is a self-contained `Design → DataSimulator → Protocol → Results` family implementing the rank-1 (Legendre-weighted) synthetic-SPAM randomized benchmarking protocol (SSR1RB) of "Randomized Benchmarking with Synthetic Quantum Circuits" for an arbitrary-spin SU(2)/qudit system. It does not reuse `vb.py`'s `SummaryStatistics`/fitting machinery beyond subclassing `BenchmarkingDesign` for its design; probabilities are simulated directly against `pygsti.tools.su2tools.SpinJ` representations rather than through a full pyGSTi `Model`. It depends on two `pygsti.tools` modules: `wignersymbols.py` (exact Clebsch-Gordan/Wigner 6-j) and `su2tools.py` (the `SpinJ` representation class). `SU2RBDataSimulator`'s `noise_channel` accepts a fixed unitary/superoperator applied after every gate, or a callable factory `(alpha, beta, gamma) -> channel` invoked per gate with that gate's own Euler angles, for gate-dependent noise.
...
| [`SU2RBDesign`](../../pygsti/protocols/su2rb.py#L230) | su2rb.py:230 | Synthetic-SPAM RB design for an arbitrary-spin SU(2)/qudit system; always uses the hidden-first-gate convention. |
| [`SU2RBDataSimulator`](../../pygsti/protocols/su2rb.py#L442) | su2rb.py:442 | `DataSimulator` that simulates SU(2) RB circuits directly against a `SpinJ` representation (no full `Model`); `noise_channel` may be a fixed channel or a per-gate callable factory. |
| [`SyntheticSPAMRank1RB`](../../pygsti/protocols/su2rb.py#L911) | su2rb.py:911 | Protocol implementing the rank-1 (Legendre-weighted) synthetic-SPAM RB (SSR1RB): per-irrep decay fits and rate recovery. |
...
| [`SyntheticSPAMRBResults`](../../pygsti/protocols/su2rb.py#L1110) | su2rb.py:1110 | Results container for `SyntheticSPAMRank1RB` (`rates_dataframe()`, `variance_diagnostic()`). |
