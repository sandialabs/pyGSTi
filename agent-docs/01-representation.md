...
> **TENTATIVE (branch `feature-instrument-seed-tools`, not yet merged).** The decomposed `(E_k, G_k)` become a **frozen static base** for Lindblad gate parameterizations, and the base's rank / effect-span / spectral-interval structure is an invariant of the whole fit — an ideal projective seed hard-caps the reachable member rank (construction now warns). Two new sibling modules address this: [`modelmembers/instruments/diagnostics.py`](../pygsti/modelmembers/instruments/diagnostics.py) (data-free O(1) checks plus a sampled Jacobian certificate; `diagnose_instrument`, `diagnose_model_instruments`) and [`modelmembers/instruments/seeding.py`](../pygsti/modelmembers/instruments/seeding.py) (repairs: `full_rank_base_blend`, `interior_effect_offset`, `displace_base_effects`, one-call `patch_instrument_seed`, and `project_instrument_to_cptp` with an SDP/Dykstra backend). Protocol-level, `pygsti.protocols.refit_instruments_cptplnd` warm-starts a CPTPLND fit from the CPTP projection of an existing non-Lindblad estimate. See [docs/markdown/objects/Instruments.md](../docs/markdown/objects/Instruments.md) for the workflow.
