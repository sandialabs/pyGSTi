...
## 18. C/A basis-element-label pair order: model objects vs. `errorgenpropagation`

**What it is.** An elementary error generator of type `C` or `A` is indexed by an *unordered* pair of Paulis, `C_{P,Q} = C_{Q,P}` and `A_{P,Q} = -A_{Q,P}`, but every label class stores the pair as an ordered tuple, and pyGSTi has no single convention for which order that is:

- **The propagation module** requires the *string-sorted* order (`'I' < 'X' < 'Y' < 'Z'` over the `'I'`-padded strings, [`bel_less_than`](../pygsti/errorgenpropagation/localstimerrorgen.py)): [`LocalStimErrorgenLabel`](../pygsti/errorgenpropagation/localstimerrorgen.py#L183) hashes and compares on its string form, so the two orders would be two different dict keys for one generator (and hide a sign for `A`). Its constructor does not check; `cast` raises `ValueError` on an out-of-order pair; and the module canonicalises at its entry points with [`canonicalize_errorgen_layer`](../pygsti/errorgenpropagation/localstimerrorgen.py#L80) (`ErrorGeneratorPropagator.construct_errorgen_layers` and `__init__`'s `fixed_errorgen_layer`, the `approximate_stabilizer_*` recasts in `tools/errgenproptools.py`), folding the `A` sign into the rate.
- **The model objects** hand out whatever order embedding produced. [`EmbeddedOp._embed_labels`](../pygsti/modelmembers/operations/embeddedop.py#L694) (and its twin in [`embeddederrorgen.py`](../pygsti/modelmembers/operations/embeddederrorgen.py)) writes a gate's locally sorted pair character by character into the gate's *target positions*, so a two-qubit gate on permuted targets (`Gcphase` on `(1, 0)`) yields e.g. `C('YI', 'IX')` with `'IX' < 'YI'`; [`LocalElementaryErrorgenLabel.cast(global, sslbls)`](../pygsti/modelmembers/operations/lindbladerrorgen.py#L1028) in `LindbladErrorgen.coefficients(label_type='local')` does the same, and [`GlobalElementaryErrorgenLabel(sort=True)`](../pygsti/baseobjs/errorgenlabel.py#L241) sorts characters within each label to the sorted `sslbls` but never the pair. `pygsti.extras.ml.errgentools.canonical_pauli_pair` is a third, independent implementation of the same string order.

**Where it bites.** Any code that takes `errorgen_coefficients(label_type='local')` from a model and uses the labels as dictionary keys alongside labels produced elsewhere (propagated labels, the `errgenproptools` commutator/composition/anticommutator emitters, a `CompleteElementaryErrorgenBasis`, which *is* string-sorted) can hold one generator under two keys, with the `A` sign wrong for one of them. Inside the propagation module this is handled at the boundary (Part IV Phase 21 of the error-generator refactor, 2026-09); everywhere else it is not. Before Phase 21 the last gate layer and the POVM layer — which `_propagate_errorgen_layers` appends *unpropagated* ([errorpropagator.py:753](../pygsti/errorgenpropagation/errorpropagator.py#L753)) — carried the model's order into every downstream sum.

**Why it is not fixed in the model.** Labels and values are paired *positionally* (`zip(embedded_labels, coeffs.values())` in `errorgen_coefficients`), so reordering an `A` label at the source requires a per-label sign threaded through `errorgen_coefficients_array`, its derivative, `set_errorgen_coefficients`, `error_rates`, the global↔local casts and user-supplied `lindblad_error_coeffs` dictionaries — and consumers outside propagation (FOGI, reports) are in the regression surface. The intended end state is one canonical pair order defined by the label classes themselves and honoured by the model; until then, **canonicalise at your own boundary** (`canonicalize_errorgen_layer` for rate dictionaries; do not construct a `LocalStimErrorgenLabel` with an out-of-order pair).

**Tracker.** No tracker — please open one.
