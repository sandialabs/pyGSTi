...
class TestCoreMethods(AlgorithmsBase):
    ...
    def test_eLGST(self):
        ...
        _,mdl_single_exlgst = pygsti.do_exlgst(ds, mdl_clgst, self.elgstStrings[0], self.fiducials, self.fiducials,
                                            self.model, regularizeFactor=1e-3, svd_truncate_to=4,
                                              verbosity=0)
        ...
        mdl_exlgst = pygsti.do_iterative_exlgst(ds, mdl_clgst, self.fiducials, self.fiducials, self.elgstStrings,
                                               target_model=self.model, svd_truncate_to=4, verbosity=0)
        ...
        all_minErrs, all_gs_exlgst_tups = pygsti.do_iterative_exlgst(
            ...
            target_model=self.model, svd_truncate_to=4, verbosity=0, returnAll=True, returnErrorVec=True)

        mdl_exlgst_verb = self.runSilent(pygsti.do_iterative_exlgst, ds, mdl_clgst, self.fiducials, self.fiducials, self.elgstStrings,
                                        target_model=self.model, svd_truncate_to=4, verbosity=10)
    ...
    def test_MC2GST(self):
        ...
        all_minErrs, all_gs_lsgst_tups = pygsti.do_iterative_mc2gst(
            ...
            min_prob_clip_for_weighting=1e-6, prob_clip_interval=(-1e6,1e6), returnAll=True, returnErrorVec=True)
        ...
        mdl_lsgst_reg = self.runSilent(pygsti.do_iterative_mc2gst,ds, mdl_clgst,
                                      ...
                                      min_prob_clip_for_weighting=1e-6,
    ...
    def test_MLGST(self):
        ...
        mdl_single_mlgst = pygsti.do_mlgst(ds, mdl_clgst, self.lsgstStrings[0], min_prob_clip=1e-4,
                                          prob_clip_interval=(-1e2,1e2), verbosity=0)

        #this test often gives an assetion error "finite Jacobian has inf norm!" on Travis CI Python 3 case
        try:
            mdl_single_mlgst_cpsp = pygsti.do_mlgst(ds, mdl_clgst, self.lsgstStrings[0], min_prob_clip=1e-4,
                                                  prob_clip_interval=(-1e2,1e2), cptp_penalty_factor=1.0,
        ...
        try:
            mdl_single_mlgst_cp = pygsti.do_mlgst(ds, mdl_clgst, self.lsgstStrings[0], min_prob_clip=1e-4,
                                                  prob_clip_interval=(-1e2,1e2), cptp_penalty_factor=1.0,
                                                  verbosity=10)
        except ValueError: pass # ignore when assertions in customlm.py are disabled
        except AssertionError:
        ...
        try:
            mdl_single_mlgst_sp = pygsti.do_mlgst(ds, mdl_clgst, self.lsgstStrings[0], min_prob_clip=1e-4,
                                                  prob_clip_interval=(-1e2,1e2), spam_penalty_factor=1.0,
                                                  verbosity=10)
        except ValueError: pass # ignore when assertions in customlm.py are disabled
        except AssertionError:
            pass # just ignore for now.  FUTURE: see what we can do in custom LM about scaling large jacobians...
        ...
        maxLogL, all_gs_mlegst_tups = pygsti.do_iterative_mlgst(
            ds, mdl_clgst, [ [mdl.tup for mdl in gsList] for gsList in self.lsgstStrings],
            min_prob_clip=1e-4, prob_clip_interval=(-1e2,1e2), returnAll=True, returnMaxLogL=True)
        ...
        #self.runSilent(pygsti.do_mlgst, ds, mdl_clgst, self.lsgstStrings[0], min_prob_clip=1e-6,
        #                prob_clip_interval=(-1e2,1e2), verbosity=4, memLimit=curMem+8500000) #invoke memory control

        #non-Poisson picture - should use (-1,-1) model for consistency?
        with self.assertRaises(NotImplementedError):
            # Non-poisson picture needs support for a non-leastsq solver (not impl yet)
            pygsti.do_mlgst(ds, mdl_clgst, self.lsgstStrings[0], min_prob_clip=1e-4,
                            prob_clip_interval=(-1e2,1e2), verbosity=0, poissonPicture=False)
            try:
                pygsti.do_mlgst(ds, mdl_clgst, self.lsgstStrings[0], min_prob_clip=1e-1, # 1e-1 b/c get inf Jacobians...
                                prob_clip_interval=(-1e2,1e2), verbosity=0, poissonPicture=False,
        ...
        with self.assertRaises(MemoryError):
            pygsti.do_mlgst(ds, mdl_clgst, self.lsgstStrings[0], min_prob_clip=1e-4,
                            prob_clip_interval=(-1e2,1e2),verbosity=0, memLimit=1)
    ...
    def test_model_selection(self):
        ...
        mdl_lgst4 = pygsti.do_lgst(ds, self.fiducials, self.fiducials, self.model, svd_truncate_to=4, verbosity=0)
        mdl_lgst6 = pygsti.do_lgst(ds, self.fiducials, self.fiducials, self.model, svd_truncate_to=6, verbosity=0)
        sys.stdout.flush()

        self.runSilent(pygsti.do_lgst, ds, self.fiducials, self.fiducials, self.model, svd_truncate_to=6, verbosity=4) # test verbose prints
        ...
        mdl_lsgst = self.runSilent(pygsti.do_iterative_mc2gst_with_model_selection, ds, mdl_lgst4, 1, self.lsgstStrings[0:3],
                                  verbosity=10, min_prob_clip_for_weighting=1e-3, prob_clip_interval=(-1e5,1e5))
        ...
        errorVecs, mdl_lsgst_wts = self.runSilent(pygsti.do_iterative_mc2gst_with_model_selection, ds, mdl_lgst4,
                                                 1, tuple_strings, verbosity=10, min_prob_clip_for_weighting=1e-3,
                                                 prob_clip_interval=(-1e5,1e5), circuit_weightsDict={ ('Gx',): 2.0 },
        ...
        mdl_non_iterative = self.runSilent( pygsti.do_mc2gst_with_model_selection, ds,
                                           ...
                                           verbosity=10, prob_clip_interval=(-1e5,1e5) )
