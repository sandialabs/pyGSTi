...
class RBTestCase(BaseTestCase):
    ...
    def test_sample(self):
        ...
        layer = rb.sample.circuit_layer_by_co2Qgates(pspec_1, None, co2Qgates, co2Qgatesprob=[1.,0.], twoQprob=1.0, 
                                                   oneQgatenames=['Gx',], modelname='clifford')
        self.assertEqual(len(layer), n_1)
        self.assertEqual(layer[0].name, 'Gx')
    ...
    def test_rb_theory(self):
        ...
        depol_strength = 1e-3
        mdl = target_model.depolarize(op_noise=depol_strength)
    
        # Tests AGI function and p_to_r with AGI.
        AGI = pygsti.tools.average_gate_infidelity(mdl.operations['Gc0'],target_model.operations['Gc0'])
        ...
        EI = pygsti.tools.entanglement_infidelity(mdl.operations['Gc0'],target_model.operations['Gc0'])
        ...
        AGsI = rb.theory.gateset_infidelity(mdl,target_model,itype='AGI')
        r_AGI = rb.analysis.p_to_r(1-depol_strength,d=2,rtype='AGI')
        r_pred_AGI = rb.theory.predicted_RB_number(mdl,target_model,rtype='AGI')
        ...
        AEI = rb.theory.gateset_infidelity(mdl,target_model,itype='EI')
        r_EI = rb.analysis.p_to_r(1-depol_strength,d=2,rtype='EI')
        r_pred_EI = rb.theory.predicted_RB_number(mdl,target_model,rtype='EI')
        ...
        mdl_in_RB_gauge = rb.theory.transform_to_rb_gauge(mdl, target_model, eigenvector_weighting=0.5)
        AEI = rb.theory.gateset_infidelity(mdl,target_model,itype='EI')
        self.assertLess(np.abs(AEI-r_EI), 10**(-10))
        ...
        for key in target_model.operations.keys():
            mdl.operations[key] = np.dot(Zrot_channel,target_model.operations[key])
        ...
        AEI = rb.theory.gateset_infidelity(mdl_in_RB_gauge,target_model,itype='EI')
        r_pred_EI = rb.theory.predicted_RB_number(mdl,target_model,rtype='EI')
        self.assertLess(np.abs(AEI-r_pred_EI), 10**(-10))
        ...
        mdl.operations['Gx'] = np.dot(depmap_X,target_model.operations['Gx'])
        mdl.operations['Gy'] = np.dot(depmap_Y,target_model.operations['Gy'])
    
        Gx_weight = 1
        Gy_weight = 2
        weights = {'Gx':Gx_weight,'Gy':Gy_weight}
        WAEI = rb.theory.gateset_infidelity(mdl,target_model,weights=weights,itype='EI')
        GxAEI = pygsti.tools.entanglement_infidelity(mdl.operations['Gx'],target_model.operations['Gx'])
        GyAEI = pygsti.tools.entanglement_infidelity(mdl.operations['Gy'],target_model.operations['Gy'])
        ...
        mdl_in_RB_gauge = rb.theory.transform_to_rb_gauge(mdl, target_model, weights=weights,
                                                         eigenvector_weighting=0.5)
        WAEI = rb.theory.gateset_infidelity(mdl_in_RB_gauge,target_model,weights=weights,itype='EI')
        # Checks the predicted RB number function works with specified weights
        r_pred_EI = rb.theory.predicted_RB_number(mdl,target_model,weights=weights,rtype='EI')
        ...
        clifford_group = rb.group.construct_1Q_Clifford_group()
        R = rb.theory.R_matrix(target_model, clifford_group, group_to_model=group_to_model, weights=weights)
    
        # Tests the p-prediction function works, and that we get the correct predictions from the R-matrix.
        p = rb.theory.R_matrix_predicted_RB_decay_parameter(target_model, clifford_group, 
                                                            group_to_model=group_to_model, 
                                                            weights=weights)
        self.assertLess(abs(p - 1.) ,  10**(-10))
        depol_strength = 1e-3
        mdl = target_model.depolarize(op_noise=depol_strength)
        p = rb.theory.R_matrix_predicted_RB_decay_parameter(mdl, clifford_group, 
                                                            group_to_model=group_to_model, 
                                                            weights=weights)
        ...
        m, ASPs = rb.theory.exact_RB_ASPs(target_model, clifford_group, m_max=10, m_min=0, m_step=1, success_outcomelabel=('0',), 
                                          group_to_model=group_to_model, weights=None, compilation=clifford_compilation, 
                                          group_twirled=False)
        ...
        depol_strength = 1e-3
        mdl = target_model.depolarize(op_noise=depol_strength)
        m, ASPs = rb.theory.exact_RB_ASPs(mdl, clifford_group, m_max=10, m_min=0, m_step=1, success_outcomelabel=('0',), 
                                          group_to_model=group_to_model, weights=None, compilation=clifford_compilation, 
                                          group_twirled=False)
        self.assertLess(abs(ASPs[0] - 1) ,  10**(-10))
    
        m, ASPs = rb.theory.exact_RB_ASPs(mdl, clifford_group, m_max=10, m_min=0, m_step=3, success_outcomelabel=('0',), 
                                          group_to_model=group_to_model, weights=weights, compilation=clifford_compilation, 
                                          group_twirled=True)
        ...
        L_m, L_ASPs, L_LASPs, L_UASPs = rb.theory.L_matrix_ASPs(mdl, target_model, m_max=10, m_min=0, m_step=1, 
        ...
        L_m, L_ASPs = rb.theory.L_matrix_ASPs(mdl, target_model, m_max=10, m_min=0, m_step=1, success_outcomelabel=('0',),
    ...
    def test_clifford_compilations(self):
        ...
        target_model = std1Q_XY.target_model.copy()
        clifford_compilation = std1Q_XY.clifford_compilation
        compiled_cliffords = pygsti.construction.build_alias_model(target_model,clifford_compilation)
