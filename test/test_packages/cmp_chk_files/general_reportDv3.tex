...
Gi & \vcenteredhbox{\includegraphics[width=1.27in,height=1.27in,keepaspectratio]{general_reportDv3_files/targetGatesBoxes_Gi.pdf}} \\ \hline
Gx & \vcenteredhbox{\includegraphics[width=1.27in,height=1.27in,keepaspectratio]{general_reportDv3_files/targetGatesBoxes_Gx.pdf}} \\ \hline
Gy & \vcenteredhbox{\includegraphics[width=1.27in,height=1.27in,keepaspectratio]{general_reportDv3_files/targetGatesBoxes_Gy.pdf}} \\ \hline
\end{tabular}

}
\end{minipage}
\caption{\pdftooltip{{\color{blue}\texttt{.tables['targetSpamBriefTable']}}\quad}{Access this information in pyGSTi via<ResultsObject>.tables['targetSpamBriefTable']}\pdftooltip{{\color{blue}\texttt{.tables['targetGatesBoxTable']}}\quad}{Access this information in pyGSTi via<ResultsObject>.tables['targetGatesBoxTable']}\textbf{Target gate set: SPAM (state preparation and measurement) and logic gates}.  The \textbf{left} table shows the \emph{ideal} input state ($\rho_0$) and `plus' POVM effect $E_0$ for the device on which we report.  SPAM gates are given here as $d\times d$ matrices.  The \textbf{right} table shows the \emph{ideal} (generally unitary) logic gates.  Each has a name starting with ``G'', and is represented as a $d^2\times d^2$ \emph{superoperator} that acts by matrix multiplication on vectors in $\mathcal{B}(\mathcal{H})$.  Matrices are displayed using a heat map that ranges between 1.0 (red) and -1.0 (blue).\label{targetGatesetTable}}
...
\caption{\pdftooltip{{\color{blue}\texttt{.tables['fiducialListTable']}}\quad}{Access this information in pyGSTi via<ResultsObject>.tables['fiducialListTable']}\textbf{Fiducial sequences.}  A list of the preparation and measurement ``fiducial'' gate sequences. See discussion in text.\label{fiducialListTable}}
\end{center}
\end{table}
...
\caption{\pdftooltip{{\color{blue}\texttt{.tables['germList2ColTable']}}\quad}{Access this information in pyGSTi via<ResultsObject>.tables['germList2ColTable']}\textbf{Germ sequences.}  A list of the ``germ'' gate sequences.  See discussion in text.\label{germListTable}}
\end{center}
\end{table}
...
\caption{\pdftooltip{{\color{blue}\texttt{.tables['datasetOverviewTable']}}\quad}{Access this information in pyGSTi via<ResultsObject>.tables['datasetOverviewTable']}\textbf{General dataset properties}.  See discussion in text.\label{datasetOverviewTable}}
\end{center}
\end{table}
...
\caption{\pdftooltip{{\color{blue}\texttt{.tables['bestGatesetGaugeOptParamsTable']}}\quad}{Access this information in pyGSTi via<ResultsObject>.tables['bestGatesetGaugeOptParamsTable']}\textbf{Gauge Optimization Details}.  A list of the parameters used when performing the gauge optimization that produced the final GST results found in subsequent tables and figures.\label{bestGatesetGaugeParamsTable}}
\end{center}
\end{table}
...
\caption{\pdftooltip{{\color{blue}\texttt{.tables['bestGatesetSpamBriefTable']}}\quad}{Access this information in pyGSTi via<ResultsObject>.tables['bestGatesetSpamBriefTable']}\textbf{The GST estimate of the SPAM operations}.  Compares the estimated SPAM operations to those desired (repeated from Table \ref{targetGatesetTable} for convenience).\label{bestGatesetSpamTable}}
\end{center}
\end{table}
...
Gi & \vcenteredhbox{\includegraphics[width=2.00in,height=2.00in,keepaspectratio]{general_reportDv3_files/targetGatesBoxes_Gi.pdf}} & \vcenteredhbox{\includegraphics[width=2.00in,height=2.00in,keepaspectratio]{general_reportDv3_files/bestGatesBoxes_Gi.pdf}} \\ \hline
Gx & \vcenteredhbox{\includegraphics[width=2.00in,height=2.00in,keepaspectratio]{general_reportDv3_files/targetGatesBoxes_Gx.pdf}} & \vcenteredhbox{\includegraphics[width=2.00in,height=2.00in,keepaspectratio]{general_reportDv3_files/bestGatesBoxes_Gx.pdf}} \\ \hline
Gy & \vcenteredhbox{\includegraphics[width=2.00in,height=2.00in,keepaspectratio]{general_reportDv3_files/targetGatesBoxes_Gy.pdf}} & \vcenteredhbox{\includegraphics[width=2.00in,height=2.00in,keepaspectratio]{general_reportDv3_files/bestGatesBoxes_Gy.pdf}} \\ \hline
\end{tabular}

\caption{\pdftooltip{{\color{blue}\texttt{.tables['bestGatesetGatesBoxTable']}}\quad}{Access this information in pyGSTi via<ResultsObject>.tables['bestGatesetGatesBoxTable']}\textbf{The GST estimate of the logic gate operations}.  Compares the \emph{ideal} (generally unitary) logic gates (second column, also in Table \ref{targetGatesetTable}) with those \emph{estimated} by GST (third column).  Each gate is represented as a $d^2\times d^2$ \emph{superoperator} that acts by matrix multiplication on vectors in $\mathcal{B}(\mathcal{H})$.  Matrices are displayed using a heat map that ranges between 1.0 (red) and -1.0 (blue).  Note that it is impossible to discern even order-$1\%$ deviations from the ideal using this color scale, and one should rely on other analysis for more a precise comparison.\label{bestGatesetGatesTable}}
% REMOVE See Table \ref{bestGatesetGatesTable} for GST estimates of the actual logic gates implemented in this experiment.
\end{center}
\end{table}
...
Gi & $ \begin{array}{c} 0.04 \\ \pm 0.003 \end{array} $ & $ \begin{array}{c} 0.04 \\ \pm 0.003 \end{array} $ & $ \begin{array}{c} 0.04 \\ \pm 0.003 \end{array} $ \\ \hline
...
\caption{\pdftooltip{{\color{blue}\texttt{.tables['bestGatesetVsTargetTable']}}\quad}{Access this information in pyGSTi via<ResultsObject>.tables['bestGatesetVsTargetTable']}\textbf{Comparison of GST estimated gates to target gates}.  This table presents, for each of the gates, three different measures of distance or discrepancy from the GST estimate to the ideal target operation.  See text for more detail.\label{bestGatesetVsTargetTable}}
\end{center}
\end{table}
...
$\rho_{0}$ & $ \begin{array}{c} 0.049 \\ \pm 0.003 \end{array} $ & $ \begin{array}{c} 0.049 \\ \pm 0.003 \end{array} $ \\ \hline
$E_{0}$ & $ \begin{array}{c} 0.045 \\ \pm 0.004 \end{array} $ & $ \begin{array}{c} 0.045 \\ \pm 0.004 \end{array} $ \\ \hline
$E_C$ & $ \begin{array}{c} 0.046 \\ \pm 0 \end{array} $ & $ \begin{array}{c} 0.046 \\ \pm 0 \end{array} $ \\ \hline
\end{tabular}

\caption{\pdftooltip{{\color{blue}\texttt{.tables['bestGatesetSpamVsTargetTable']}}\quad}{Access this information in pyGSTi via<ResultsObject>.tables['bestGatesetSpamVsTargetTable']}\textbf{Comparison of GST estimated SPAM to target SPAM}.  This table presents, for each state preparation and POVM effect, two different measures of distance or discrepancy from the GST estimate to the ideal target operation.  See text for more detail.\label{bestGatesetSpamVsTargetTable}}
\end{center}
\end{table}
...
Gi & \vcenteredhbox{\includegraphics[width=2.00in,height=2.00in,keepaspectratio]{general_reportDv3_files/bestErrgenBoxes_Gi_errgen.pdf}} & \vcenteredhbox{\includegraphics[width=1.56in,height=1.56in,keepaspectratio]{general_reportDv3_files/bestErrgenBoxes_Gi_hamdecomp.pdf}} & \vcenteredhbox{\includegraphics[width=1.56in,height=1.56in,keepaspectratio]{general_reportDv3_files/bestErrgenBoxes_Gi_stodecomp.pdf}} \\ \hline
Gx & \vcenteredhbox{\includegraphics[width=2.00in,height=2.00in,keepaspectratio]{general_reportDv3_files/bestErrgenBoxes_Gx_errgen.pdf}} & \vcenteredhbox{\includegraphics[width=1.56in,height=1.56in,keepaspectratio]{general_reportDv3_files/bestErrgenBoxes_Gx_hamdecomp.pdf}} & \vcenteredhbox{\includegraphics[width=1.56in,height=1.56in,keepaspectratio]{general_reportDv3_files/bestErrgenBoxes_Gx_stodecomp.pdf}} \\ \hline
Gy & \vcenteredhbox{\includegraphics[width=2.00in,height=2.00in,keepaspectratio]{general_reportDv3_files/bestErrgenBoxes_Gy_errgen.pdf}} & \vcenteredhbox{\includegraphics[width=1.56in,height=1.56in,keepaspectratio]{general_reportDv3_files/bestErrgenBoxes_Gy_hamdecomp.pdf}} & \vcenteredhbox{\includegraphics[width=1.56in,height=1.56in,keepaspectratio]{general_reportDv3_files/bestErrgenBoxes_Gy_stodecomp.pdf}} \\ \hline
\end{tabular}

\caption{\pdftooltip{{\color{blue}\texttt{.tables['bestGatesetErrGenBoxTable']}}\quad}{Access this information in pyGSTi via<ResultsObject>.tables['bestGatesetErrGenBoxTable']}\textbf{The GST estimate of the logic gate operation generators}. A heat map of the ``Error Generator'' for each gate, which is the Lindbladian $\mathbb{L}$ that describes \emph{how} the gate is failing to match the target.  This error generator is defined by the equation $\hat{G} = G_{\mathrm{target}}e^{\mathbb{L}}$. When all elements of these matrices is zero, the estimated gates match the target gates (Table \ref{targetGatesetTable}).  Note that the range of the color scale is variable, being determined by the data.  In the third column, each generator is projected onto each of the ``Hamiltonian generators'' given by the action of commutation with each Pauli-product basis element.  In the forth column, each generator is projected onto each of the ``Stochastic generators'' given by the action of conjugation with each Pauli-product basis element.  Columns and rows correspond to Pauli operators on the first and second (if present) qubit.\label{bestGatesetErrGenTable} }
\end{center}
\end{table}

It's usually useful to understand \emph{how} gates fail.  The error generators in Table \ref{bestGatesetErrGenTable} provide one view on this, but they are not necessarily intuitive.   For example, you might want to know whether your gate suffers depolarizing, dephasing, or over-rotation errors.  In Tables \ref{bestGatesetEigenvalueTable}, the eigenvalues of the estimated gates are shown and plotted on the complex disc (which can be helpful for visualizing the different types of errors, e.g., over- or under-rotation vs.~depolarization).  Table \ref{bestGatesetRelativeEigenvalueTable} shows the same quantities but for the eigenvalues of $\hat{G} = G_{\mathrm{target}}e^{\mathbb{L}}$, which would all be $1.0$ if the gate were ideal.  %Table \ref{bestGatesetPolarEigvalTable} plots these eigenvalues on the complex disc, which can be helpful for visualizing the different types of errors (e.g.~over- or under-rotation vs.~depolarization). 
...
0.946e^{-i0.00} & 1
 \end{array} $
 & \vcenteredhbox{\includegraphics[width=2.00in,height=2.00in,keepaspectratio]{general_reportDv3_files/bestEvalPolarPlt_Gi.pdf}} \\ \hline
Gx & $ \begin{array}{cc}
0.949e^{i1.57} & 0.949e^{-i1.57} \\ 
0.951 & 1
 \end{array} $
 & \vcenteredhbox{\includegraphics[width=2.00in,height=2.00in,keepaspectratio]{general_reportDv3_files/bestEvalPolarPlt_Gx.pdf}} \\ \hline
Gy & $ \begin{array}{cc}
0.949e^{i1.57} & 0.949e^{-i1.57} \\ 
0.951 & 1
 \end{array} $
 & \vcenteredhbox{\includegraphics[width=2.00in,height=2.00in,keepaspectratio]{general_reportDv3_files/bestEvalPolarPlt_Gy.pdf}} \\ \hline
\end{tabular}

\caption{\pdftooltip{{\color{blue}\texttt{.tables['bestGatesetEvalTable']}}\quad}{Access this information in pyGSTi via<ResultsObject>.tables['bestGatesetEvalTable']}\textbf{Eigenvalues of estimated gates}.  The spectrum (Eigenvalues column) of each estimated gate.  The second column displays these eigenvalues over the complex disc.  \textbf{Black} and \textcolor{cyan}{\textbf{cyan}} circles plot the spectrum of the \textbf{ideal target gate} and \textcolor{cyan}{\textbf{estimated gate}}, respectively.\label{bestGatesetEigenvalueTable}}
\end{center}
\end{table}
...
0.056e^{-i3.11} & 0
 \end{array} $
 & \vcenteredhbox{\includegraphics[width=2.00in,height=2.00in,keepaspectratio]{general_reportDv3_files/bestRelEvalPolarPlt_Gi.pdf}} \\ \hline
Gx & $ \begin{array}{cc}
0 & 0.051e^{i3.12} \\ 
0.051e^{-i3.12} & -0.054
 \end{array} $
 & \vcenteredhbox{\includegraphics[width=2.00in,height=2.00in,keepaspectratio]{general_reportDv3_files/bestRelEvalPolarPlt_Gx.pdf}} \\ \hline
Gy & $ \begin{array}{cc}
0 & -0.049 \\ 
-0.052 & -0.053
 \end{array} $
 & \vcenteredhbox{\includegraphics[width=2.00in,height=2.00in,keepaspectratio]{general_reportDv3_files/bestRelEvalPolarPlt_Gy.pdf}} \\ \hline
\end{tabular}

\caption{\pdftooltip{{\color{blue}\texttt{.tables['bestGatesetEvalTable']}}\quad}{Access this information in pyGSTi via<ResultsObject>.tables['bestGatesetEvalTable']}\textbf{Relative Eigenvalues of estimated gates}.  For each gate, the spectrum of $G_{\mathrm{target}}^{-1}\hat{G}$, defined as its \emph{relative eigenvalues} (see text for details).  The second column displays these eigenvalues over the complex disc.  Small \textcolor{red}{\textbf{red}} circles plot the \textcolor{red}{\textbf{relative eigenvalues}}, and \textcolor{darkgreen}{\textbf{green}} circles plot the relative eigenvalues of the gate \textcolor{darkgreen}{\textbf{to the 10th power}} (to increase visibility).\label{bestGatesetRelativeEigenvalueTable}}
\end{center}
\end{table}
...
0.003
 \end{array} $
 & \vcenteredhbox{\includegraphics[width=2.53in,height=2.53in,keepaspectratio]{general_reportDv3_files/bestChoiEvalBars_Gi.pdf}} \\ \hline
Gx & $ \begin{array}{c}
0.011 \\ 
0.013 \\ 
0.014 \\ 
...
0.002
 \end{array} $
 & \vcenteredhbox{\includegraphics[width=2.53in,height=2.53in,keepaspectratio]{general_reportDv3_files/bestChoiEvalBars_Gx.pdf}} \\ \hline
Gy & $ \begin{array}{c}
0.011 \\ 
0.012 \\ 
0.015 \\ 
...
0.002
 \end{array} $
 & \vcenteredhbox{\includegraphics[width=2.53in,height=2.53in,keepaspectratio]{general_reportDv3_files/bestChoiEvalBars_Gy.pdf}} \\ \hline
\end{tabular}

\caption{\pdftooltip{{\color{blue}\texttt{.tables['bestGatesetChoiEvalTable']}}\quad}{Access this information in pyGSTi via<ResultsObject>.tables['bestGatesetChoiEvalTable']}\textbf{Choi matrix spectrum of the GST estimated gate set}.  The eigenvalues of the Choi representation of each estimated gate.  In the third column, magnitudes of \emph{negative} values are plotted using \textcolor{red}{red} bars. Unitary gates have a spectrum $(1,0,0\ldots)$, just like pure quantum states.  Negative eigenvalues are non-physical, and may represent either statistical fluctuations or violations of the CPTP model used by GST.\label{bestGatesetChoiTable}}
\end{center}
\end{table}
...
\begin{figure}
\begin{center}
\includegraphics[width=1.80in,height=1.80in,keepaspectratio]{general_reportDv3_files/colorBoxPlotKey.png}
\caption{\pdftooltip{{\color{blue}\texttt{.figures['colorBoxPlotKeyPlot']}}\quad}{Access this information in pyGSTi via<ResultsObject>.figures['colorBoxPlotKeyPlot']}\textbf{Sub-block key for subsequent plots.} Shows how elements of the sub-blocks in Figure \ref{bestEstimateColorBoxPlot} correspond to preparation and measurement fiducial sequences.  Note that the column indicates the fiducial adjacent to state preparation, while the row indicates the fiducial adjacent to measurement.\label{colorBoxPlotKeyPlot}}
\end{center}
\end{figure}
...
\caption{\pdftooltip{{\color{blue}\texttt{.tables['metadataTable']}}\quad}{Access this information in pyGSTi via<ResultsObject>.tables['metadataTable']}\textbf{Listing of GST parameters and meta-data.}  These parameters and related metadata describe how the GST computation was performed which led to this report. \label{metadataTable}}
\end{center}
\end{table}
...
\caption{\pdftooltip{{\color{blue}\texttt{.tables['softwareEnvTable']}}\quad}{Access this information in pyGSTi via<ResultsObject>.tables['softwareEnvTable']}\textbf{Listing of the software environment.}  Note that this describes the software environment of the machine used to generate this report, and not necessarily the machine used to perform the core GST gate set estimatino.\label{metadataTable}}
\end{center}
\end{table}
