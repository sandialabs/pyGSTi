...
class GateConstructionTestCase(BaseTestCase):
    def test_CNOT_convention(self):
        ...
        std_cnot = pygsti.tools.std_process_mx_to_unitary(pygsti.tools.change_basis(std2Q_XYICNOT.target_model().operations['Gcnot'].to_dense('minimal'), 'pp', 'std'))
        state_10 = pygsti.tools.dmvec_to_state(pygsti.tools.change_basis(std2Q_XYICNOT.target_model().povms['Mdefault']['10'].to_dense('minimal'), "pp", "std"))
