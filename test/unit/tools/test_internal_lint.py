...
def check_file(fname: str) -> bool:
    ...
    if os.path.exists(fname):
        ...
        for i, line in enumerate(data):
            error = error or check_line(line, i+1, fname) # line numbers in editors start at 1.
...
def check_for_implied_zeros(line: str, ind: int, fname: str) -> bool:
    ...
    missing_implied_zero_pattern = re.compile("(?P<LeadingPunctuation>[ ,({])(?P<LeadingTerm>[^a-zA-Z0-9%]*)[.](?P<PostDecimal>[0-9][0-9]*)(?P<Punctuation>[ ,)}])")
...
def test_repo_against_internal_styling():
    ...
    if error:
        raise ValueError(" There has been an error." \
        "Check the printed statements for the location.")