...
class WriteDatasetTester(WritersBase):
    def setUp(self):
        ...
        self.ds = io_gen._ds
        self.fixture_path = self.fixture_data('dataset_loadwrite.txt')
    ...
    def test_write_dataset(self, tmp_path):
        ...
        self.assertFilesEquivalent(tmp_path, self.fixture_data('dataset_loadwrite.txt'))
...
class WriteSparseDatasetTester(WritersBase):
    ...
    def test_write_sparse_dataset(self, tmp_path):
        ...
        self.assertFilesEquivalent(tmp_path, self.fixture_data('sparse_dataset1a.txt'))
        writers.write_dataset(tmp_path, self.ds, outcomeLabelOrder=None, fixedColumnMode=False)
        self.assertFilesEquivalent(tmp_path, self.fixture_data('sparse_dataset2a.txt'))
    ...
    def test_write_sparse_dataset_ordered(self, tmp_path):
        ...
        self.assertFilesEquivalent(tmp_path, self.fixture_data('sparse_dataset1b.txt'))
        writers.write_dataset(tmp_path, self.ds, outcomeLabelOrder=ordering, fixedColumnMode=False)
        self.assertFilesEquivalent(tmp_path, self.fixture_data('sparse_dataset2b.txt'))


class WriteMultidatasetTester(WritersBase):
    def setUp(self):
        self.circuit_list = io_gen._circuit_list
        self.fixture_path = self.fixture_data('TestMultiDataset.txt')
        # TODO generate dynamically
        self.mds = io.load_multidataset(str(self.fixture_path))
    ...
    def test_write_multidataset(self, tmp_path):
        ...
        self.assertFilesEquivalent(tmp_path, self.fixture_path)
        # TODO fix failing
...
class WriteCircuitListTester(WritersBase):
    def setUp(self):
        ...
        self.fixture_path = self.fixture_data('gatestringlist_loadwrite.txt')
...
class WriteModelTester(WritersBase):
    def setUp(self):
        ...
        self.fixture_path = self.fixture_data('gateset_loadwrite.txt')

    @with_temp_path
    def test_write_model_no_identity(self, tmp_path):
        writers.write_model(io_gen._std_model_no_identity, tmp_path)
        self.assertFilesEquivalent(tmp_path, self.fixture_data('gateset_noidentity.txt'))
