...
class ToExplicitModelTester(BaseCase):
    ...
    def setUp(self):
        self.pspec = QubitProcessorSpec(2, ('Gx', 'Gy', 'Gcnot'), geometry="line",
                                        qubit_labels=['qb0', 'qb1'])
        self.test_circuits = [
            Circuit([('Gx', 'qb0'), ('Gy', 'qb1')], line_labels=('qb0', 'qb1')),
            Circuit([('Gcnot', 'qb0', 'qb1')], line_labels=('qb0', 'qb1')),
            Circuit([('Gx', 'qb0'), ('Gcnot', 'qb0', 'qb1'), ('Gy', 'qb1'), ('Gx', 'qb1')],
                    line_labels=('qb0', 'qb1')),
            Circuit([('Gy', 'qb0'), ('Gy', 'qb0'), ('Gcnot', 'qb1', 'qb0')], line_labels=('qb0', 'qb1')),
        ]
    ...
    def test_returns_explicit_model_with_expected_members(self):
        mdl = create_crosstalk_free_model(self.pspec, ideal_gate_type='H+S',
        ...
        self.assertEqual(set(exp.operations.keys()), set(mdl.operation_blks['layers'].keys()))
        self.assertEqual(set(exp.preps.keys()), set(mdl.prep_blks['layers'].keys()))
        self.assertEqual(set(exp.povms.keys()), set(mdl.povm_blks['layers'].keys()))
    ...
    def test_preserves_parameterization(self):
        # An H+S model has parameterized (error-generator) operations; conversion
    ...
    def test_preserves_parameter_sharing(self):
        ...
        shared = create_crosstalk_free_model(self.pspec, ideal_gate_type='H+S',
                                             independent_gates=False)
        indep = create_crosstalk_free_model(self.pspec, ideal_gate_type='H+S',
        ...
        # sharing genuinely reduces the parameter count (and survives conversion)
    ...
    def test_handles_instruments(self):
        # Instruments in (crosstalk-free) implicit models act on all qudits, so
        # they are keyed without subsystem labels and stored full-dimension.
    ...
    def test_raises_on_factories(self):
        mdl = create_crosstalk_free_model(self.pspec, ideal_gate_type='static')
