...
def germ_irrep_multiplicities(germ_superop, order, tol=1e-8):
    """
    The multiplicity of each irrep of `Z_order` in an ideal germ's superoperator.

    The ideal germ generates a cyclic group of the given order, so its
    superoperator is diagonalizable with eigenvalues among the characters
    `chi_j(1) = exp(2 pi i j / order)`.  The multiplicity of irrep `j` is the
    dimension of the eigenspace with that eigenvalue, i.e. the dimension of
    the block that character filtering on irrep `j` isolates.  For the trivial
    irrep this includes the identity (fixed-point) direction, so a
    single-qubit rotation germ has trivial multiplicity 2 (identity plus the
    rotation axis) and multiplicity 1 on each of its two conjugate Ramsey
    irreps.  Irreps the germ does not occupy are absent from the result.

    Parameters
    ----------
    germ_superop : numpy.ndarray
        The (dense) superoperator matrix of the ideal germ.

    order : int
        The order of the cyclic group generated by the ideal germ (see
        :func:`germ_group_order`).

    tol : float, optional
        Tolerance for matching an eigenvalue to a character value.

    Returns
    -------
    dict
        Maps irrep index (int in `[0, order)`) to multiplicity (int).
    """
    evals = _np.linalg.eigvals(_np.asarray(germ_superop, dtype=complex))
    out = {}
    for j in range(order):
        chi = _np.exp(2j * _np.pi * j / order)
        mult = int(_np.count_nonzero(_np.abs(evals - chi) < tol))
        if mult > 0:
            out[j] = mult
    if sum(out.values()) != germ_superop.shape[0]:
        raise ValueError("Germ eigenvalues are not all %d-th roots of unity (within tol=%g); "
                         "is `order` the germ's group order?" % (order, tol))
    return out
