...
class TensorProductPOVMEffect(_POVMEffect):
        ...
        for p in _lt.partition_into(order, len(self.factors)):
            ...
            for factors in _itertools.product(*factor_lists):
                ...
                pre_state_rep = self._evotype.create_tensorproduct_state_rep(
                    [self._evotype.create_pure_state_rep(f.pre_effect.to_dense(SpaceConversionType.Hilbert), None,
                ...
                post_state_rep = self._evotype.create_tensorproduct_state_rep(
                    [self._evotype.create_pure_state_rep(f.post_effect.to_dense(SpaceConversionType.Hilbert), None,
    ...
    def deriv_wrt_params(self, wrt_filter=None):
        ...
        for i, (fct, fct_local_inds, fct_dim) in enumerate(zip(self.factors, self._submember_rpindices, dims)):
            ...
            if i > 0:  # factors before ith
                pre = self.factors[0][self.effectLbls[0]].to_dense(SpaceConversionType.Minimal)
                for j, fctA in enumerate(self.factors[1:i], start=1):
                    pre = _np.kron(pre, fctA[self.effectLbls[j]].to_dense(SpaceConversionType.Minimal))
                deriv = _np.kron(pre[:, None], deriv)  # add a dummy 1-dim to 'pre' and do kron properly...

            if i + 1 < len(self.factors):  # factors after ith
                post = self.factors[i + 1][self.effectLbls[i + 1]].to_dense(SpaceConversionType.Minimal)
                for j, fctA in enumerate(self.factors[i + 2:], start=i + 2):
                    post = _np.kron(post, fctA[self.effectLbls[j]].to_dense(SpaceConversionType.Minimal))
    ...
    def __str__(self):
        ...
        # factors are POVMs
        s += " x ".join([_mt.mx_to_string(fct[self.effectLbls[i]].to_dense(SpaceConversionType.Minimal), width=4, prec=2)
