...
class TPInstrumentOp(_DenseOperator, _Torchable):
    ...
    def _construct_matrix(self):
        ...
        if self.index < nEls - 1:
            self._ptr[:, :] = (self.relevant_param_ops[1].to_dense('minimal')  # i.e. param_ops[self.index + 1]
                               + self.relevant_param_ops[0].to_dense('minimal'))  # i.e. param_ops[0]
        else:
            assert(self.index == nEls - 1), \
                "Invalid index %d > %d" % (self.index, nEls - 1)
            self._ptr[:, :] = (-sum(op.to_dense('minimal') for op in self.relevant_param_ops)  # all instrument param_ops == relevant
                               - (nEls - 3) * self.relevant_param_ops[0].to_dense('minimal'))
