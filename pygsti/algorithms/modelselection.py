    ...
    while not exceeded_threshold:
        ...
        if best_model[1] > er_thresh or len(best_model[0]) == 1:
            ...
            if recompute_Hessian:
                recompute_Hessian = False
                temp_model = None
                parent_model_projector = np.eye(len(expansion_point_x0))
                ...
                #TODO if assertion below is not raised get rid of parent_model_projector and replace with deltalogl_projector
                assert np.allclose(deltalogl_model_projector, parent_model_projector), 'We cannot get rid of this variable'
