...
def create_design_matrix_list(circuit_list,
                              model,
                              model_parameter_indexing,
                              usePaulis=False,
                              pauli_measurements=None,
                              include_spam=False,
    ...
    if show_progress_bar:
        ...
        for i, circuit in enumerate(tqdm(circuit_list,
...
def create_design_matrix_list_parallel_pathos(
    circuit_list,
    model,
    model_parameter_indexing,
    usePaulis=False,
    ...
    """
    Calculates design matrices for a list of circuits, optionally in parallel with progress bar.

    Parameters:
    - circuit_list: list of circuits
    - model: pygsti Noise model
    - model_parameter_indexing: list of error generator tuples and gates
    - usePaulis: bool, whether to use Pauli measurement outcomes
    - pauli_measurements: optional, Pauli measurements
    - include_spam: bool, placeholder for SPAM inclusion
    - show_progress_bar: bool, whether to show tqdm progress bar
    - do_parallel: bool, whether to run in parallel or serially
    - num_workers: int, number of parallel workers (only used if do_parallel=True)

    Returns:
    - design_matrices: list of design matrices in the order of circuit_list
    """


    if do_parallel:
        # Import pathos only if parallel execution is requested
        from pathos.multiprocessing import ProcessingPool as Pool

        with Pool(nodes=num_workers) as pool:
        # pool = Pool(nodes=num_workers)
            ...
            for batch_idx in range(num_batches):
                ...
                end = min(len(circuit_list), (batch_idx+1)*batch_size)
                batches.append(circuit_list[start:end])

            from functools import partial


            task_fn = partial(
                create_design_matrix_list,
                model=model,
                model_parameter_indexing=model_parameter_indexing,
                usePaulis=usePaulis,
            ...
            if show_progress_bar:
                from tqdm import tqdm
                nested_results = list(tqdm(pool.imap(task_fn, batches),
                                           ascii=True,
                                           desc="Processing circuit batches into design matrix",
                                           total=len(batches)))
            else:
                nested_results = pool.map(task_fn, batches)

            
        design_matrices = list(_itertools.chain.from_iterable(nested_results))

            # if show_progress_bar:
            #     # Import tqdm only if progress bar is requested
            #     from tqdm import tqdm
            #     # Use imap for lazy evaluation with progress bar
            #     # results_iter = pool.imap(worker_func, circuit_list)
            #     # design_matrices = []
            #     # for dm in tqdm(results_iter, total=len(circuit_list), desc="Processing circuits"):
            #     design_matrices = tqdm(pool.imap(worker_func, circuit_list), total=len(circuit_list), desc="Processing circuits")
            #     # pool.close()
            #     # pool.join()
            # else:
            #     # No progress bar, just map
            #     design_matrices = pool.map(worker_func, circuit_list)
            #     # pool.close()
            #     # pool.join()
    else:
        design_matrices = create_design_matrix_list(circuit_list=circuit_list,
                                                    model=model,
                                                    model_parameter_indexing=model_parameter_indexing,
                                                    usePaulis=usePaulis,
                                                    pauli_measurements=pauli_measurements,
        ...
        # if show_progress_bar:
        #     from tqdm import tqdm
        #     design_matrices = [worker_func(circuit) for circuit in tqdm(circuit_list, desc="Processing circuits")]
        # else:
        #     design_matrices = [worker_func(circuit) for circuit in circuit_list]
