    ...
    if method == "ls":
        ...
        def _jacobian_fn(gauge_group_el):
            ...
            for lbl, G in mdl_pre.operations.items():
                ...
                left = -1 * _np.dot(dS, mdl_post.operations[lbl].to_dense(SpaceConversionType.Minimal))  # shape (n,d1,d2)
                right = _np.swapaxes(_np.dot(G.to_dense(SpaceConversionType.Minimal), dS), 0, 1)  # shape (d1,n,d2) -> (n,d1,d2)
            ...
            for ilbl, Inst in mdl_pre.instruments.items():
                ...
                for lbl, G in Inst.items():
                    ...
                    left = -1 * _np.dot(dS, mdl_post.instruments[ilbl][lbl].to_dense(SpaceConversionType.Minimal))  # (n,d1,d2)
                    right = _np.swapaxes(_np.dot(G.to_dense(SpaceConversionType.Minimal), dS), 0, 1)  # (d1,n,d2) -> (n,d1,d2)
                    result = _np.swapaxes(_np.dot(S_inv, left + right), 1, 2)  # shape (d1, d2, n)
                    result = result.reshape((d**2, n))  # must copy b/c non-contiguous
                    my_jacMx[start:start + d**2] = wt * result
                    start += d**2
            ...
            for lbl, rho in mdl_post.preps.items():
                ...
                result = -1 * _np.dot(Sinv_dS, rho.to_dense(SpaceConversionType.Minimal))  # shape (d,n)
            ...
            for povmlbl, povm in mdl_pre.povms.items():
                for lbl, E in povm.items():
                    ...
                    result = _np.dot(E.to_dense(SpaceConversionType.Minimal)[None, :], dS).T  # shape (1,n,d2).T => (d2,n,1)
...
def _spam_penalty_jac_fill(spam_penalty_vec_grad_to_fill, mdl_pre, mdl_post,
    ...
    for i, (lbl, prepvec) in enumerate(mdl_post.preps.items()):
        ...
        # dmDim = denMx.shape[0]
        denMx = _tools.vec_to_stdmx(prepvec.to_dense(SpaceConversionType.Minimal)[:, None], op_basis)
        ...
        Sinv_dS = _np.dot(S_inv, dS)  # shape (d1,n,d2)
        dVdp = -1 * _np.dot(Sinv_dS, prepvec.to_dense(SpaceConversionType.Minimal)[:, None]).squeeze(2)  # shape (d,n,1) => (d,n)
    ...
    for povmlbl, povm in mdl_post.povms.items():
        for lbl, effectvec in povm.items():
            ...
            EMx = _tools.vec_to_stdmx(effectvec.to_dense(SpaceConversionType.Minimal)[:, None], op_basis)
            # dmDim = EMx.shape[0]
            ...
            #                      = [effectvec.T * dS].T
            #  OR = dS.T * effectvec
            pre_effectvec = mdl_pre.povms[povmlbl][lbl].to_dense(SpaceConversionType.Minimal)[:, None]
