    ...
    while d_ind < d:
        ...
        if len(layer) > 0 and layer[0].name == zrotname:  # ask if it's a Zrot layer.
            ...
            net_pauli_numbers = _symp.find_pauli_number(_symp.symplectic_rep_of_clifford_circuit(
                _cir.Circuit(new_paulis_as_layer + net_paulis_as_layer), srep_dict=srep_dict)[1])
            ...
            if circ_type == 'clifford+zxzxz':
                ...
                net_paulis = {qubits[i]: pn for i, pn in enumerate(_symp.find_pauli_number(
                    _symp.symplectic_rep_of_clifford_circuit(_cir.Circuit([layer, net_paulis_as_layer, layer]),
