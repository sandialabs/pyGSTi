...
def active_jac_element(prep, error, meas, outcome):
    """
    Computes the Jacobian matrix element for a active error: how the
    probability of `outcome` changes with respect to the rate of `error`
    when preparing state `prep` and measuring in basis `meas`.

    Note: active error maps leave qubits corresponging to I's in
    `error` alone.  An active error is defined as replacing
    ...
    error : NQPauliOp
        The type (as a pauli operator) of active error.
    ...
    float
    """
    # Note an error of 'ZI' does *not* mean the "ZI active error":
    #   rho -> (Id[rho] + eps*actZI[rho]) = rho + eps*ZI
    #   where ZI = diag(1,1,-1,-1), so this adds prob to 00 and 01 and removes from 10 and 11.
    # Instead it means the map actZ x Id where actZ : rho -> rho + eps Z and Id : rho -> rho.
    ...
    def _acthelper(
        ...
        using POVM (meas_sign,meas_basis), and experiences an active error given
        (at this qubit) by Pauli "error_pauli", then at what rate does that change probability of outcome "bit"?
        This is going to get multiplied over all qubits.  A zero indicates that the active error is orthogonal
        to the measurement basis, which means the probability of *all* outcomes including this bit are unaffected.

        Returns 0, +1, or -1.
        """
        # Specifically, this computes Tr( (I+/-P) actErr[ (I+/-P) ] ) where the two
        # P's represent the prep & measure bases (and can be different).  Here actErr
        ...
        if error_pauli == "I":  # special case: no active action on this space
...
def active_jac_obs_element(prep, error, observable):
    """
    Computes the Jacobian matrix element for a active error: how the
    ...
    # Note: as in active_jac_element, 'I's in error mean that this active error
    # doesn't act (acts as the identity) on that qubit.

    def _acthelper(prep_sign, prep_basis, error_pauli, obs_pauli):
        assert prep_basis in ("X", "Y", "Z")  # 'I', for instance, is invalid

        # want Tr(obs_pauli * actErr[ I+/-P ] ).  There are several cases:
        # 1) if obs_pauli == 'I':
        #   - if error_pauli == 'I' (so actErr = Id), Tr(I +/- P) == 1 always
