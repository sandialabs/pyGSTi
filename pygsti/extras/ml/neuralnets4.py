...
class CircuitErrorVec(_keras.Model):
    def __init__(self, num_qubits: int, num_channels: int, tracked_error_gens: list, 
        ...
        dense_units: list
            Make sure list ends in num_tracked_error_gens
    ...
    def calc_masked_err_rates(self, signed_M, P, mask):
        n_batch = signed_M.shape[0]
        # print('signed_M', signed_M.shape, 'P', P.shape, 'mask', mask.shape)
    
        # Expand mask to match the shape of signed_M
        mask_expanded = tf.expand_dims(mask, axis=0)  # Shape (1, 132)
        mask_expanded = tf.tile(mask_expanded, [tf.shape(signed_M)[0], 1])  # Shape (n_batch, 132)
    
        # Apply the mask to signed_M and P
        masked_M = tf.math.multiply(tf.cast(mask, tf.float32), signed_M)
        masked_P = tf.math.multiply(mask, P)
    
        # Flatten the last two dimensions for both masked_M and masked_P
        flat_masked_M = tf.reshape(masked_M, [-1, masked_M.shape[1]*masked_M.shape[2]])  # Shape (n_batch, 180 * 132,)
        flat_masked_P = tf.reshape(masked_P, [-1])  # Shape (n_batch, 180 * 132,)
    
        # Get unique values and their indices
        # unique_masked_P, idx = tf.unique(flat_masked_P)  # Unique values and indices
        # num_segments = tf.reduce_max(idx, axis=1) + 1  # Number of unique segments
    
        # # Calculate the unsorted segment sum
        # segment_sum = tf.math.unsorted_segment_sum(flat_masked_M, idx, num_segments)  # Shape (num_segments,)
    
        # # We need to return a tensor of size (n_batch,)
        # # To do this, we can sum the segment sums for each batch
        # # We will need to reshape segment_sum to (n_batch, -1) and then sum along the second axis
        # segment_sum_reshaped = tf.reshape(segment_sum, [tf.shape(signed_M)[0], -1])  # Shape (n_batch, num_segments)
        
        # # Sum along the second axis to get a scalar for each batch
        # result = tf.reduce_sum(segment_sum_reshaped, axis=1)  # Shape (n_batch,)
    
        # return result
        return tf.reduce_sum(flat_masked_M, axis=1)
    ...
    def calc_end_of_circ_error(self, M, P, S):
        ...
        """
        # print('M', M.shape, 'P', P.shape, 'S', S.shape)
        ...
        # print('final_stochastic_error_rates', final_stochastic_error_rates.shape, 'final_hamiltonian_error_rates', final_hamiltonian_error_rates.shape)
        return final_stochastic_error_rates + tf.square(final_hamiltonian_error_rates)
    ...
    def call(self, inputs):
        out = self.circuit_to_fidelity(inputs)
        print('in', inputs.shape, 'out', out.shape)
        return out# will try to further optimize this with batching rather than slow map_fn / vectorize_map
...
class DenseSubNetwork(_keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        outputs = self.output_layer(self.sequential(inputs))
        # print('dense_call_inputs', inputs.shape, 'dense_call_outputs', outputs.shape)

        return outputs

# @_keras.utils.register_keras_serializable()
class LocalizedDenseToErrVec(_keras.layers.Layer):
    def __init__(self, layer_snipper, layer_snipper_args, tracked_error_gens, dense_units = [30, 20, 10, 5, 5, 132], **kwargs):
        """
        layer_snipper: func
            A function that takes a primitive error generator and maps it to a list that encodes which parts
            of a circuit layer to `snip out` as input to dense neural network that predicts the error rate
        ...
        self.indices_tensor = tf.stack(self.layer_encoding_indices_for_error_gen)
        self.dense_units = dense_units
        self.layer_snipper = layer_snipper
        self.layer_snipper_args = layer_snipper_args
    ...
    def compute_output_shape(self, input_shape):
        ...
        return self.output_shape #(None, input_shape[1], self.num_tracked_error_gens)
    ...
    def call(self, inputs):
        ...
        # Expand dimensions to match the new batch size
        batch_size_new = tf.shape(inputs)[0]  # n_batch_new
        batch_size = tf.shape(inputs)[1]       # n_batch

        indices_tiled = tf.tile(tf.expand_dims(self.indices_tensor, 0), [batch_size_new*batch_size, 1, 1])
        
        # print('input_shape', inputs.shape, 'indices_tensor', self.indices_tensor.shape, 'indices_tiled', indices_tiled.shape)

        # Gather the values based on the indices
        # Here we need to gather along the second dimension (n_batch)
        # gathered_slices = tf.gather(inputs, indices_tiled, batch_dims=2)
        gathered_slices = tf.reshape(tf.gather(tf.reshape(inputs, (batch_size_new*batch_size, 44)), indices_tiled, batch_dims=1), (batch_size_new, batch_size, 132, 44))
        print(gathered_slices.shape)
        # Reshape the gathered slices to concatenate along the last axis
        # The new shape should be [n_batch_new, -1] to flatten the gathered slices
        # gathered_slices = tf.reshape(gathered_slices, [batch_size_new, batch_size, -1])

        # Pass the concatenated slices through the dense layer
        # We need to reshape it to [n_batch_new * n_batch, -1] for the dense layer
        gathered_slices = tf.reshape(gathered_slices, [-1, batch_size, 132*44])
        x = self.dense(gathered_slices)
        
        # Reshape the output back to [n_batch_new, n_batch, output_dim] if needed
        # output_dim = x.shape[-1]  # Get the output dimension from the dense layer
        # x = tf.reshape(x, [batch_size_new, batch_size, output_dim])
        self.output_shape = (None, *x.shape[1:])
        return x
