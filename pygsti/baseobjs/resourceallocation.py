    ...
    def gather_base(self, result, local, slice_of_global, unit_ralloc=None, all_gather=False):
        ...
        if gather_comm is None or gather_comm.size == 1:
            ...
            if all_gather:
                ...
                gather_comm.Allgatherv(local.flatten() if participating else _np.empty(0, dtype=local.dtype),
                                       (gathered_data, sizes))
            else:
                ...
                gather_comm.Gatherv(local.flatten() if participating else _np.empty(0, dtype=local.dtype),
                                    recvbuf, root=0)
