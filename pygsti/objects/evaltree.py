...
class EvalTree(list):
    ...
    def create(cls, circuits_to_evaluate):  # a class method instead of __init__ because we inherit from list
        ...
        for k in indices_sorted_by_circuit_len:
            ...
            if len(eval_tree) >= 2 * len(circuits_to_evaluate):
                _warnings.warn(("Created an evaluation tree that is inefficient: %d circuits => tree of size %d!\n"
                                "Consider using a different formward simulator (e.g. MapForwardSimulator).")
                               % (len(circuits_to_evaluate), len(eval_tree)))
