...
from .kivymodalcursor import CursorModalView
...
class ResizableBehavior(object):
    ...
    def on_mouse_move(self, _, pos):
        ...
        #print("RZ mouse move pos = ", pos, ' my pos', self.pos, ' size', self.size, ' local', local_pos)
        if self.resize_lock:
            return

        if self.cursor and self.cursor.grabbed_by is None:
            oldhover = self.hovering_resizable
            self.hovering_resizable = self.check_resizable_side(local_pos[0], local_pos[1])
    ...
    def resize_widget(self, touch):
        ...
        #print("RZ: Pos",self.pos," size",self.size,' touch',touch.pos)
