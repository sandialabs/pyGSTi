    ...
    for i, rhoVec in enumerate(rho_vecs):
        ...
        for povm in povms:
            for EVec in povm.values():
                ret[i, j] = _np.vdot(EVec.to_dense("HilbertScmidt"),
                                     rhoVec.to_dense("HilbertScmidt"))
                j += 1
...
class GateEigenvalues(_modf.ModelFunction):
    ...
    def evaluate(self, model):
        ...
        numpy.ndarray
        """
        evals, evecs = _np.linalg.eig(model.operations[self.oplabel].to_dense("HilbertScmidt"))
...
def angles_btwn_rotn_axes(model):
    ...
    for i, gl in enumerate(opLabels):
        decomp = _tools.decompose_gate_matrix(model.operations[gl].to_dense("HilbertScmidt"))
    ...
    class HalfDiamondNorm(_modf.ModelFunction):
        ...
        def __init__(self, model_a, model_b, oplabel):
            ...
            if isinstance(model_b, _ExplicitOpModel):
                self.B = model_b.operations[oplabel].to_dense("HilbertScmidt")
            else:
                self.B = model_b.operation_blks['gates'][oplabel].to_dense("HilbertScmidt")
            self.d = int(round(_np.sqrt(model_a.dim)))
        ...
        def evaluate(self, model):
            ...
            if isinstance(model, _ExplicitOpModel):
                dm, W = _tools.diamonddist(model.operations[gl].to_dense("HilbertScmidt"),
                                           self.B, model.basis, return_x=True)
            else:
                dm, W = _tools.diamonddist(model.operation_blks['gates'][gl].to_dense("HilbertScmidt"),
        ...
        def evaluate_nearby(self, nearby_model):
            ...
            if isinstance(nearby_model, _ExplicitOpModel):
                A = nearby_model.operations[self.oplabel].to_dense("HilbertScmidt")
            else:
                A = nearby_model.operation_blks['gates'][self.oplabel].to_dense("HilbertScmidt")
...
def general_decomposition(model_a, model_b):
    ...
    for gl in opLabels:
        gate = model_a.operations[gl].to_dense("HilbertScmidt")
        targetOp = model_b.operations[gl].to_dense("HilbertScmidt")
