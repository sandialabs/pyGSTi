...
def _nested_color_boxplot(plt_data_list_of_lists, colormap,
    ...
    if dofs_list_of_lists is not None: 
        msg = "data and associated degrees of freedom must have the same shape!"
        assert (_np.shape(dofs_list_of_lists)==_np.shape(plt_data_list_of_lists)), msg
...
def _circuit_color_boxplot(circuit_structure, sub_mxs, colormap,
    ...
    if sub_dofs is not None:
        addl_hover_submxs['effective d.o.f.'] = sub_dofs

    if hover_info:
        if isinstance(g, _PlaquetteGridCircuitStructure):
            hover_info = _create_hover_info_fn(circuit_structure, circuit_structure.used_xs, circuit_structure.used_ys, 
                                               sum_up, addl_hover_submxs)
