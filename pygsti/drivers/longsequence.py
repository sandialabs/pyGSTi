...
def do_model_test(model_filename_or_object,
                  ...
                  prep_strs_list_or_filename, effect_strs_list_or_filename,
    ...
    prep_strs_list_or_filename : (list of Circuits) or string
    ...
    effect_strs_list_or_filename : (list of Circuits) or string or None
        ...
        then use the same strings as specified by prep_strs_list_or_filename.
    ...
    proto = _proto.ModelTest(_load_model(model_filename_or_object), None, gopt_suite, None,
                             advanced_options, output_pkl, printer)
    return proto.run(data, mem_limit, comm)
...
def do_linear_gst(data_filename_or_set, target_model_filename_or_object,
    ...
    exp_design = _proto.StandardGSTDesign(target_model, prep_strs_list_or_filename, effect_strs_list_or_filename,
                                          germs, maxLengths,
...
def do_long_sequence_gst_base(data_filename_or_set, target_model_filename_or_object,
    ...
    if isinstance(lsgst_lists, validStructTypes) or isinstance(lsgst_lists[0], validStructTypes):
...
def legacy_do_model_test(model_filename_or_object,
    ...
    prepStrs, effectStrs, germs = _load_fiducials_and_germs(
        prep_strs_list_or_filename,
        effect_strs_list_or_filename,
...
def legacy_do_long_sequence_gst(data_filename_or_set, target_model_filename_or_object,
    ...
    return legacy_do_long_sequence_gst_base(ds, target_model, lsgstLists, gauge_opt_params,
                                            advanced_options, comm, mem_limit,
                                            output_pkl, printer)
...
def legacy_do_long_sequence_gst_base(data_filename_or_set, target_model_filename_or_object,
    ...
    validStructTypes = (_objs.LsGermsStructure, _objs.LsGermsSerialStructure)
    if isinstance(lsgst_lists, validStructTypes):
        master = lsgst_lists
    ...
    if isinstance(lsgst_lists[0], validStructTypes) and LGSTcompatibleOps:
        startingPt = advanced_options.get('starting point', "LGST")
    else:
        startingPt = advanced_options.get('starting point', "target")
    ...
    if startingPt == "LGST":
        assert(isinstance(lsgst_lists[0], validStructTypes)), \
            "Cannot run LGST: fiducials not specified!"
        opLabels = advanced_options.get('opLabels',
                                       list(target_model.operations.keys())
                                       + list(target_model.instruments.keys()))
        mdl_start = _alg.do_lgst(ds, lsgst_lists[0].prepStrs, lsgst_lists[0].effectStrs, target_model,
    ...
    aliases = lsgst_lists[-1].aliases if isinstance(
        lsgst_lists[-1], validStructTypes) else None
        ...
        for i, mode in enumerate(modes):
            ...
            if mode == "Target":
                ...
                ret = legacy_do_model_test(target_model, ds, tgt, prepStrs,
                                           effectStrs, germs, max_lengths, False, advanced,
                ...
                tgt.default_gauge_group = _objs.TrivialGaugeGroup(tgt.dim)  # so no gauge opt is done
                advanced.update({'appendTo': ret, 'estimateLabel': est_label})
                ret = legacy_do_model_test(models_to_test[mode], ds, tgt, prepStrs,
                                           effectStrs, germs, max_lengths, False, advanced,
                                           comm, mem_limit, None, printer - 1)
            else:
            ...
            gaugeOptSuite_dict = gaugeopt_suite_to_dictionary(gauge_opt_suite, tgt,
...
def gaugeopt_suite_to_dictionary(gauge_opt_suite, target_model, advanced_options=None, verbosity=0):
    ...
    if isinstance(gauge_opt_suite, dict):
        ...
        if isinstance(gauge_opt_suite, str):
            gaugeOptSuites = [gauge_opt_suite]
        else:
            gaugeOptSuites = gauge_opt_suite[:]  # assumes gauge_opt_suite is a list/tuple of strs
        ...
        for suiteName in gaugeOptSuites:
            if suiteName == "single":
                ...
                assert(any([nm in ("single", "varySpam", "varySpamWt", "varyValidSpamWt", "toggleValidSpam")
                            for nm in gauge_opt_suite])), "'unreliable2Q' suite must be used with a spam or single suite."

            elif suiteName == "none":
                pass  # add nothing

            else:
                raise ValueError("Invalid `gauge_opt_suite` argument - unknown suite '%s'" % suiteName)
...
def _load_fiducials_and_germs(prep_strs_list_or_filename,
                              effect_strs_list_or_filename,
                              germs_list_or_filename):

    if isinstance(prep_strs_list_or_filename, str):
        prepStrs = _io.load_circuit_list(prep_strs_list_or_filename)
    else: prepStrs = prep_strs_list_or_filename

    if effect_strs_list_or_filename is None:
        effectStrs = prepStrs  # use same strings for effectStrs if effect_strs_list_or_filename is None
    else:
        if isinstance(effect_strs_list_or_filename, str):
            effectStrs = _io.load_circuit_list(effect_strs_list_or_filename)
        else: effectStrs = effect_strs_list_or_filename
...
def _get_lsgst_lists(dschk, target_model, prep_strs, effect_strs, germs,
                     max_lengths, advanced_options, verbosity):
    """
    ...
    actionIfMissing = advanced_options.get('missingDataAction', 'drop')
    opLabels = advanced_options.get(
        'opLabels', list(target_model.get_primitive_op_labels()))
    lsgstLists = _construction.stdlists.make_lsgst_structs(
        opLabels, prep_strs, effect_strs, germs, max_lengths,
        ...
        verbosity=verbosity)
    assert(len(max_lengths) == len(lsgstLists))
...
def _package_into_results(caller_name, ds, target_model, mdl_start, lsgst_lists,
    ...
    badFitThreshold = advanced_options.get('badFitThreshold', DEFAULT_BAD_FIT_THRESHOLD)
    onBadFit = advanced_options.get('onBadFit', [])  # ["wildcard"]) #["Robust+"]) # empty list => 'do nothing'
    add_badfit_estimates(ret, estlbl, onBadFit, badFitThreshold, opt_args, evaltree_cache, comm, mem_limit, printer - 1)
...
def add_gauge_opt(estimate, gauge_opt_params, target_model, starting_model,
    ...
    if isinstance(gauge_opt_params['model'], _objs.ExplicitOpModel):
        ...
        _, gaugeEl, go_gs_final = _alg.gaugeopt_to_target(**gauge_opt_params)
    else:
        #but still fill in results for other models (?)
        gaugeEl, go_gs_final = None, gauge_opt_params['model'].copy()

    gauge_opt_params['_gaugeGroupEl'] = gaugeEl  # store gaugeopt el
...
def get_robust_scaling(scale_typ, model, ds, circuit_list, parameters, evaltree_cache, comm, mem_limit):
    """
    Get the per-circuit data scaling ("weights") for a given type of robust-data-scaling.
    TODO: docstring - more details
    """

    fitQty = _get_fit_qty(model, ds, circuit_list, parameters, evaltree_cache, comm, mem_limit)
...
def get_wildcard_budget(model, ds, circuits_to_use, parameters, evaltree_cache, comm, mem_limit, verbosity):
    printer = _objs.VerbosityPrinter.build_printer(verbosity, comm)
    fitQty = _get_fit_qty(model, ds, circuits_to_use, parameters, evaltree_cache, comm, mem_limit)
