...
from pygsti.models import ExplicitOpModel as _ExpModel
...
class ModelPack(_ABC):
    ...
    model : Model
        Target model for the model pack.
    """
    description : str = None
    model : _ExpModel = None
    ...
    def target_model(self, gate_type="full", prep_type="auto", povm_type="auto", instrument_type="auto",
        ...
        if cache_key not in self._gscache:
            # cache miss
            pspec = self.model.create_processor_spec(qubit_labels)

            mdl = _create_explicit_model(pspec, ideal_gate_type=gate_type, ideal_spam_type=)
