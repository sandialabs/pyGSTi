    ...
    def from_mirror_experiment(cls, unmirrored_design, mirrored_data,
        ...
        for c in _tqdm.tqdm(mirrored_data.dataset, ascii=True, desc='Calculating effective polarizations'):
            for edkey, ed in mirrored_data.edesign.items():
                ...
                for aux in auxlist:
                    ...
                    hdc = _hamming_distance_counts(mirrored_data.dataset[c], c, (aux['idealout'],))
                    ep = _effective_polarization(hdc)
        ...
        for i, key in enumerate(sorted(list(seen_keys), key=lambda x: x[2])):
            ...
            if 'cp' in mirrored_data.edesign and 'cpref' in mirrored_data.edesign:
                ...
                cp_pfid = _predicted_process_fidelity_for_central_pauli_mcs(eff_pols['cp'][key],
                                                                           eff_pols['cpref'][(key[0], key[3])],
                                                                           key[0])
                
                cp_pol = _fidelity_to_polarization(cp_pfid, key[0]) # should this be fidelity_to_polarization?
                cp_success_prob = _polarization_to_success_probability(cp_pol, key[0])
            ...
            if 'rr' in mirrored_data.edesign and 'br' in mirrored_data.edesign and 'ref' in mirrored_data.edesign:
                ...
                rc_pfid = _rc_predicted_process_fidelity(eff_pols['br'][key],
                                                     eff_pols['rr'][key],
                                                     eff_pols['ref'][(key[0], key[3])],
                                                     key[0])
                
                rc_pol = _fidelity_to_polarization(rc_pfid, key[0]) # should this be fidelity_to_polarization?
                rc_success_prob = _polarization_to_success_probability(rc_pol, key[0])

                if bootstrap:
                # do bootstrapping to obtain confidence intervals
                    rc_pfid_stdev = _rc_bootstrap_predicted_pfid(brs=eff_pols['br'][key],
                                                            rrs=eff_pols['rr'][key],
            ...
            for y in self.y_values:
                ...
                if metric == 'polarization':
                    ...
                    if len(set(tempdf_xy['Width'])) == 1:
                        ...
                        sp_threshold = _polarization_to_success_probability(threshold, w)
                    else:
...
def _success_probability_to_polarization(s, n):
    """
...
def _polarization_to_success_probability(p, n):
    """
    Utility function for MCFE VBDataFrame creation.
...
def _fidelity_to_polarization(f, n):
    """
    Utility function for MCFE VBDataFrame creation.
...
def _hamming_distance_counts(dsrow, circ, idealout):
    """
    Utility function for MCFE VBDataFrame creation.
...
def _effective_polarization(hamming_distance_counts):
    """
    Utility function for MCFE VBDataFrame creation.
...
def _rc_predicted_process_fidelity(bare_rc_effective_pols, rc_rc_effective_pols, reference_effective_pols, n):
    """
    Utility function for MCFE VBDataFrame creation.
...
def _predicted_process_fidelity_for_central_pauli_mcs(central_pauli_effective_pols, reference_effective_pols, n):
    """
    Utility function for MCFE VBDataFrame creation.
...
def _rc_bootstrap_predicted_pfid(brs, rrs, refs, n, num_bootstraps=50, rand_state=None):
    """
    Utility function for MCFE VBDataFrame creation.
    ...
    for _ in range(num_bootstraps):
        ...
        pfid = _rc_predicted_process_fidelity(
            br_sample,
            rr_sample,
            ref_sample,
            n)
