...
The API pages are generated by `sphinx.ext.autosummary` from pyGSTi's docstrings: roughly 1200 of them against
about 100 tutorial pages, and they account for nearly all of the build time.
...
**If you are editing the tutorials, build only the prose:**
    ``python docs/build-prose.py``

That skips the API reference entirely and finishes in seconds rather than tens of minutes; it prints the path to
the `index.html` it produced. It derives its config and table of contents from `_config.yml` and `_toc.yml` at run
time, so there is nothing to keep in sync. The build it produces has no API section in its sidebar, and a
cross-reference from a tutorial into the API pages would not resolve there — so run the full `jb build docs`
before you trust a link check, or when you need the API reference itself.

Both builds are incremental by default, re-reading only the pages that changed. Pass `--all` to force a full
re-read. Avoid `jb clean docs --all` unless you mean it: it deletes `docs/_build`, including Sphinx's `.doctrees`
cache, so the next build starts from scratch. The one case that needs it is a page that was deleted or renamed,
whose stale HTML would otherwise keep being served; deleting just that file from `docs/_build/html` is cheaper.
